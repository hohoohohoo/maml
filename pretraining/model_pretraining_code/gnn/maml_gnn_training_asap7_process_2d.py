#!/usr/bin/env python
"""
MAML GNN Training for ASAP7 Process Dataset - Unified 4-D V×T Format

Uses 2-D (V×T) datasets produced by asap7/build_gnn_dataset_process_cached_asap7_2d.py.
Each task = one (corner, cell, delay_arc, slew_idx, load_idx) — samples span
the full (61 voltage × 6 temperature) plane = 366 samples per task.
...
- MAML meta-learning with inner/outer loop, identical methodology to 1-D version
- K=5 support set sampling — randomly from the flattened 366 (V, T) samples per task
- Per-task output normalization (on-the-fly)

Dataset Structure (unified 4-D V×T format):
- train_{data_type}_{graph_mode}_2d.pth
  - node_features: [num_voltages, num_temps, total_nodes, 11] torch.Tensor
  - outputs: [num_voltages, num_temps, num_tasks] torch.Tensor
  - node_slices: [num_tasks + 1] torch.Tensor
  - cell_names, delay_types, output_names, task_corners, voltages, temperatures, norm_stats, etc.
...
def find_pretrained_gnn_model(model_dir, data_type, graph_mode, innerdiv, meta,
    ...
    # Search pattern
    pattern = f"gnn_maml_asap7_process_2d_{data_type}_{graph_mode}_innerdiv{innerdiv}_meta{meta}_iter*_inner{inner_steps}{arch_suffix}.pth"
...
class ASAP7ProcessMAMLDataset2D(Dataset):
    """
    ...
    def __init__(self, train_path, graph_mode='stage_aware', cache_path_override=None):
        ...
        print(f"ASAP7ProcessMAMLDataset2D initialized (4-D V×T format, mmap):")
        print(f"   Train file: {train_path}")
        print(f"   Tasks: {self._num_tasks}")
        print(f"   Voltages: {self._num_voltages}, Temps: {self._num_temps}")
        print(f"   Samples per task (V×T): {self._num_libs}")
    ...
    def _load_data(self):
        """Load 4-D V×T tensor data using mmap and topology cache."""
        ...
        if data_format != 'unified_4d_VT':
            raise ValueError(f"Expected unified_4d_VT format, got: {data_format}")

        # node_features: [V, T, total_nodes, F]; outputs: [V, T, num_tasks]
        ...
        self._num_tasks = int(data['num_tasks'])
        # Flattened sample count per task = V × T (e.g. 61 × 6 = 366)
        self._num_libs = self._num_voltages * self._num_temps

        self._norm_stats = data.get('norm_stats', None)

        # Optional axis coords for diagnostics
        self._voltages = data.get('voltages', None)
        self._temperatures = data.get('temperatures', None)
    ...
    def __getitem__(self, task_idx):
        """
        Return one task's V×T grid as a flat list of 366 minimal_samples.

        Each minimal_sample carries one (voltage, temperature) point's node_features
        slice and scalar output. The MAML inner loop randomly samples K of these
        366 entries — identical sampling policy to the 1-D code (random K).
        """
        ...
        node_end = (self._node_slices[task_idx + 1].item()
                    if task_idx + 1 < len(self._node_slices)
                    else self._node_features.shape[2])

        # task_outputs has shape [V, T] — flatten to length V*T
        task_outputs = self._outputs[:, :, task_idx]   # [V, T]
        task_outputs_flat = task_outputs.reshape(-1)   # [V*T]

        minimal_samples = []
        for v in range(self._num_voltages):
            for t in range(self._num_temps):
                task_node_features = self._node_features[v, t, node_start:node_end, :]
                sample = {
                    'node_features': task_node_features,
                    'edge_index': edge_index,
                    'output': float(self._outputs[v, t, task_idx].item()),
                    'cell_name': cell_name,
        ...
        return {
            ...
            'outputs': task_outputs_flat.tolist(),
            'task_idx': task_idx,
            'cell_name': cell_name,
            'delay_type': delay_type,
            'output_name': output_name,
...
DEFAULT_DATASET_DIR = "/home/tkdgn2907/Deepsets_test/MAML/Projects/dataset_all/gnn_dataset_asap7_2d"
...
"""
    ...
    parser.add_argument('--sampling', type=str, default='10pct',
                       help='Train sampling ratio suffix (e.g., "10pct" → _samp10pct in train file name). '
                            'Empty string disables sampling suffix. Default: "10pct".')
...
def train_single_config(args):
    ...
    # Construct train file path (order must match asap7/build_gnn_dataset_process_cached_asap7_2d.py)
    # Order: topology_suffix → voltage_suffix → norm_suffix → temp_suffix → slew_suffix → sampling_suffix
    ...
    train_file = f"train_{data_type}_{graph_mode}{topology_suffix}{voltage_suffix}{norm_suffix}{temp_suffix}{slew_suffix}{sampling_suffix}_2d.pth"
    ...
    dataset = ASAP7ProcessMAMLDataset2D(
    ...
    checkpoint_dir = f"../../../pretrained_models/gnn_maml_asap7_process_2d_checkpoints{checkpoint_suffix}{voltage_suffix}{norm_suffix}{temp_suffix}{slew_suffix}{sampling_suffix}"
    final_model_dir = f"../../../pretrained_models/gnn_maml_asap7_process_2d_final{checkpoint_suffix}{voltage_suffix}{norm_suffix}{temp_suffix}{slew_suffix}{sampling_suffix}"
    ...
    for idx, (conv_hidden_dim, num_conv_layers, fc_hidden_dim, num_fc_layers) in enumerate(arch_combinations, 1):
        ...
        for chunk in range(num_chunks):
            ...
            checkpoint_path = f"{checkpoint_dir}/gnn_maml_asap7_process_2d_{data_type}_{graph_mode}_innerdiv{innerdiv}_meta{tasks_per_meta_batch}_iter{cumulative_iterations}_inner{inner_steps}{arch_suffix}.pth"
        ...
        pool_suffix = f"_pool{pooling_mode}" if pooling_mode != 'mean' else ""
        arch_suffix = f"_conv{conv_hidden_dim}x{num_conv_layers}_fc{fc_hidden_dim}x{num_fc_layers}{pool_suffix}"
        final_model_path = f"{final_model_dir}/gnn_maml_asap7_process_2d_{data_type}_{graph_mode}_innerdiv{innerdiv}_meta{tasks_per_meta_batch}_iter{final_iteration}_inner{inner_steps}{arch_suffix}.pth"
        ...
        if args.enable_loss_logging and gnn_maml.iteration_loss_log:
            loss_log_dir = args.loss_log_dir or f"../../../pretrained_models/loss_logs_2d{checkpoint_suffix}{voltage_suffix}{norm_suffix}{temp_suffix}{slew_suffix}"
            os.makedirs(loss_log_dir, exist_ok=True)
            loss_log_filename = f"loss_log_2d_{data_type}_{graph_mode}_innerdiv{innerdiv}_meta{tasks_per_meta_batch}_iter{final_iteration}_inner{inner_steps}{arch_suffix}.json"
