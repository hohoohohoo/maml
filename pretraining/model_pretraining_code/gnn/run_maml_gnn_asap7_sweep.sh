#!/bin/bash
# MAML GNN Training Sweep with Unified Dataset (mmap Loading)
# Uses pre-processed unified dataset - no preprocessing needed
...
if [ $# -lt 1 ]; then
    ...
    echo ""
    echo "Example:"
    echo "  $0 json_configs/maml_gnn_sweep_config.json"
    echo "  $0 json_configs/maml_gnn_sweep_config.json --dry-run"
    echo ""
    echo "Note: Requires unified dataset to be pre-generated."
...
echo "=========================================="
echo "MAML GNN Training - Unified Dataset"
echo "=========================================="
...
if [ "$NO_COMMIT" = false ] && [ "$DRY_RUN" = false ]; then
    if git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        if git diff --cached --quiet; then
            ...
            COMMIT_MSG="Start MAML GNN unified sweep: $(basename $CONFIG_FILE)

Experiment Configuration:
- Config file: $CONFIG_FILE
- Timestamp: $TIMESTAMP
- Mode: Unified dataset (mmap loading)
...
# Build command - use unified version
FULL_CMD="python -u maml_gnn_training_asap7.py $BASE_CONFIG $SWEEP_PARAMS"
...
if [ "$DRY_RUN" = true ]; then
    ...
    echo ""
    echo "This will:"
    echo "  1. Load unified dataset with mmap (memory efficient)"
