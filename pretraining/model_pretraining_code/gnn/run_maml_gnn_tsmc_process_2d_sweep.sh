#!/bin/bash
# MAML GNN Training Sweep for TSMC Process 2-D (V×T) Dataset (unified_4d_VT format)
# Uses pre-built 2-D dataset (tsmc/build_gnn_dataset_process_cached_tsmc_2d.py)
# MAML meta-learning (NOT mini-batch baseline)
...
FULL_CMD="python -u maml_gnn_training_tsmc_process_2d.py $BASE_CONFIG $SWEEP_PARAMS $LOSS_LOGGING_CONFIG"
...
if [ "$DRY_RUN" = true ]; then
    ...
    echo "  3. Save all models to pretrained_models/gnn_maml_tsmc_process_2d_final{suffix}/"
    echo "     (suffix: _vddonly if voltage_mode=vdd_only, _relpin if related_pin_only=true)"
    echo ""
    echo "Training type: MAML meta-learning (V×T = 366 samples per task)"
