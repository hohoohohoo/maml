#!/bin/bash
# MAML GNN Training Sweep for TSMC Process Dataset (Unified 3D Format) - NORM2 Version
# Uses pre-processed TSMC unified dataset with modified normalization (norm2)
...
echo "=========================================="
echo "MAML GNN Training - TSMC Process (Norm2)"
echo "=========================================="
...
- Timestamp: $TIMESTAMP
- Dataset: TSMC Process Norm2 (11D node features: 7 base + 4 process params, modified normalization)
...
# Build command - use TSMC Process Norm2 version
FULL_CMD="python -u maml_gnn_training_tsmc_process_norm2.py $BASE_CONFIG $SWEEP_PARAMS $RESUME_CONFIG"
...
if [ "$DRY_RUN" = true ]; then
    ...
    echo ""
    echo "This will:"
    echo "  1. Load TSMC Process Norm2 dataset (11D node features, modified normalization)"
    echo "  2. Train $TOTAL_COMBINATIONS architectures sequentially"
    echo "  3. Save all models to pretrained_models/gnn_maml_tsmc_process_norm2_final[_vdd_only]/"
    echo ""
...
# Execute
echo "Starting TSMC Process Norm2 sweep..."
echo ""
echo "Command:"
echo "$FULL_CMD"
echo ""
