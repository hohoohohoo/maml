#!/usr/bin/env python
"""
MAML Heterogeneous GNN Training for TSMC Process Dataset - Unified 3D Format
...
Key Features:
- Heterogeneous GNN with type-specific transformations for different node types
  (Power, Port, NMOS, PMOS)
- Node features: 11D (7 base + 4 process params: param_a, param_b, param_c, temperature)
- Supports both GCN and GAT convolution types
...
def extract_iteration_from_hetero_gnn_filename(filename):
    """Extract iteration number from Heterogeneous GNN model filename"""
...
def find_pretrained_hetero_gnn_model(model_dir, data_type, graph_mode, innerdiv, meta,
                                      inner_steps, conv_hidden_dim, num_conv_layers,
                                      fc_hidden_dim, num_fc_layers, pooling='mean',
                                      conv_type='gcn'):
    """
    Find the latest pretrained Heterogeneous GNN model matching the given parameters.
    ...
    conv_type_suffix = f"_{conv_type}" if conv_type != 'gcn' else ""
    arch_suffix = f"_conv{conv_hidden_dim}x{num_conv_layers}_fc{fc_hidden_dim}x{num_fc_layers}{pool_suffix}{conv_type_suffix}"

    # Search pattern
    pattern = f"hetero_gnn_maml_tsmc_process_{data_type}_{graph_mode}_innerdiv{innerdiv}_meta{meta}_iter*_inner{inner_steps}{arch_suffix}.pth"
    ...
    for f in matching_files:
        iteration = extract_iteration_from_hetero_gnn_filename(f)
...
def load_pretrained_hetero_gnn_model(model, checkpoint_path, device):
    """
    Load pretrained Heterogeneous GNN model from checkpoint.
...
class HeteroGNN_MAML_TSMCProcess:
    """
    Heterogeneous GNN MAML training with TSMC Process dataset (11D features).
    Meta-learning with inner/outer loop optimization.
    Uses type-specific transformations for different node types.
    """
    ...
    def __init__(self, model, inner_lr, meta_lr, K=5, inner_steps=1,
        ...
        print(f"\nHeteroGNN MAML TSMC Process Configuration:")
        ...
        print(f"  Model type: Heterogeneous GNN")
    ...
    def main_loop_sequential(self, num_iterations):
        ...
        print(f"\nStarting TSMC Process Heterogeneous GNN MAML Training")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='MAML Heterogeneous GNN Training for TSMC Process Dataset (11D features)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Single training run
  python maml_hetero_gnn_training_tsmc_process.py --graph_mode stage_aware

  # Architecture sweep
  python maml_hetero_gnn_training_tsmc_process.py \\
      --conv_hidden_dim 32 64 128 \\
      --num_conv_layers 2 3

  # Use GAT convolution
  python maml_hetero_gnn_training_tsmc_process.py --conv_type gat

  # Custom hyperparameters
  python maml_hetero_gnn_training_tsmc_process.py --innerdiv 10 --meta_lr 0.0001 --K 5
"""
    )
    ...
    parser.add_argument('--conv_type', type=str, default='gcn',
                       choices=['gcn', 'gat'],
                       help='Convolution type: gcn (GCN-based) or gat (GAT with attention) (default: gcn)')
    parser.add_argument('--num_node_types', type=int, default=4,
                       help='Number of node types (default: 4 for Power, Port, NMOS, PMOS)')
...
def train_single_config(args):
    ...
    # Extract gatectrl suffix from cache_path for checkpoint naming
    # (gatectrl doesn't affect train data, but affects model training via adjacency)
    cache_suffix = ''
    if cache_path:
    ...
    print(f"\n{'#'*80}")
    print(f"# MAML Heterogeneous GNN Training - TSMC Process (11D features)")
    print(f"{'#'*80}")
    ...
    print(f"Convolution type: {conv_type}")
    print(f"Node types: {num_node_types} (Power, Port, NMOS, PMOS)")
    ...
    # Add conv_type suffix for GAT models
    conv_type_suffix = f"_{conv_type}" if conv_type != 'gcn' else ""
    checkpoint_dir = f"../../../pretrained_models/hetero_gnn_maml_tsmc_process_checkpoints{checkpoint_suffix}{voltage_suffix}{norm_suffix}{temp_suffix}{slew_suffix}{conv_type_suffix}"
    final_model_dir = f"../../../pretrained_models/hetero_gnn_maml_tsmc_process_final{checkpoint_suffix}{voltage_suffix}{norm_suffix}{temp_suffix}{slew_suffix}{conv_type_suffix}"
    ...
    for idx, (conv_hidden_dim, num_conv_layers, fc_hidden_dim, num_fc_layers) in enumerate(arch_combinations, 1):
        ...
        print(f"Creating Heterogeneous GNN MAML model (11D input, pooling={pooling_mode}, conv_type={conv_type})...")

        hetero_gnn_model = create_maml_hetero_gnn_model(
        ...
        if args.resume:
            ...
            if os.path.exists(args.resume):
                start_iteration = extract_iteration_from_hetero_gnn_filename(args.resume)
                loaded_model = load_pretrained_hetero_gnn_model(hetero_gnn_model, args.resume, device)
            else:
            ...
            for search_dir in search_dirs:
                model_path, iteration = find_pretrained_hetero_gnn_model(
                    search_dir, data_type, graph_mode, innerdiv,
                    tasks_per_meta_batch, inner_steps,
                    conv_hidden_dim, num_conv_layers,
                    fc_hidden_dim, num_fc_layers, pooling_mode, conv_type
                )
            ...
            if best_path:
                ...
                loaded_model = load_pretrained_hetero_gnn_model(hetero_gnn_model, best_path, device)
        ...
        hetero_gnn_maml = HeteroGNN_MAML_TSMCProcess(
            model=hetero_gnn_model,
            inner_lr=inner_lr,
            meta_lr=meta_lr,
            K=K,
        ...
        for chunk in range(num_chunks):
            ...
            try:
                hetero_gnn_maml.main_loop_sequential(num_iterations=chunk_iters)
            except Exception as e:
                print(f"Training failed: {e}")
                print("Reducing learning rates and retrying...")
                hetero_gnn_maml.inner_lr *= 0.5
                hetero_gnn_maml.meta_lr *= 0.5
                hetero_gnn_maml.main_loop_sequential(num_iterations=chunk_iters // 2)
            ...
            conv_type_suffix_file = f"_{conv_type}" if conv_type != 'gcn' else ""
            arch_suffix = f"_conv{conv_hidden_dim}x{num_conv_layers}_fc{fc_hidden_dim}x{num_fc_layers}{pool_suffix}{conv_type_suffix_file}"

            checkpoint_path = f"{checkpoint_dir}/hetero_gnn_maml_tsmc_process_{data_type}_{graph_mode}_innerdiv{innerdiv}_meta{tasks_per_meta_batch}_iter{cumulative_iterations}_inner{inner_steps}{arch_suffix}.pth"

            torch.save({
                'model_state_dict': hetero_gnn_maml.model.state_dict(),
                'norm_stats': norm_stats,
                'task_norm_stats': hetero_gnn_maml.task_norm_stats,
                'config': {
                    'data_type': data_type,
                    'graph_mode': graph_mode,
                    ...
                    'meta_losses': hetero_gnn_maml.meta_losses,
                    'node_features': 11
                }
            }, checkpoint_path)
        ...
        conv_type_suffix_file = f"_{conv_type}" if conv_type != 'gcn' else ""
        arch_suffix = f"_conv{conv_hidden_dim}x{num_conv_layers}_fc{fc_hidden_dim}x{num_fc_layers}{pool_suffix}{conv_type_suffix_file}"
        final_model_path = f"{final_model_dir}/hetero_gnn_maml_tsmc_process_{data_type}_{graph_mode}_innerdiv{innerdiv}_meta{tasks_per_meta_batch}_iter{final_iteration}_inner{inner_steps}{arch_suffix}.pth"
        ...
        # Cleanup
        del hetero_gnn_maml.task_norm_stats
        del hetero_gnn_maml.weights
        del hetero_gnn_maml.meta_optimiser
        del hetero_gnn_maml.model
        del hetero_gnn_maml
