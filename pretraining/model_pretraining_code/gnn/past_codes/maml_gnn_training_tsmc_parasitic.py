#!/usr/bin/env python
"""
MAML GNN Training for TSMC Dataset with Parasitic Capacitance - Unified 3D Format

Uses unified datasets from build_gnn_dataset_process_cached_tsmc.py (with R,C values)
Memory-mapped loading (mmap) for large datasets via torch.load(..., mmap=True).

Key Features:
- Node features: 12D (7 base + voltage + input_slew + output_load + temperature + parasitic_cap)
...
class TSMCParasiticMAMLDataset(Dataset):
    """
    PyTorch Dataset for TSMC Parasitic Cap MAML training with mmap loading.
    Uses tensor-based storage format with 12D node features.

    Data format (3D tensor):
    - node_features: [num_libs, total_nodes, 12] - mmap tensor
    ...
    def __init__(self, train_path, graph_mode='stage_aware'):
        ...
        print(f"TSMCParasiticMAMLDataset initialized (tensor format, mmap):")
...
class GNN_MAML_TSMCParasitic:
    """
    GNN MAML training with TSMC Parasitic Cap dataset (12D features).
    ...
    def __init__(self, model, inner_lr, meta_lr, K=5, inner_steps=1,
        ...
        print(f"\nGNN MAML TSMC Parasitic Configuration:")
    ...
    def main_loop_sequential(self, num_iterations):
        ...
        print(f"\nStarting TSMC Parasitic MAML Training")
...
DEFAULT_DATASET_DIR = "/home/tkdgn2907/Deepsets_test/MAML/Projects/dataset_all/dataset_TSMC_GNN_unified/with_parasitic_cap"


def parse_arguments():
    """Parse command-line arguments"""
    parser = argparse.ArgumentParser(
        description='MAML GNN Training for TSMC Parasitic Cap Dataset (12D features)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Single training run
  python maml_gnn_training_tsmc_parasitic.py --graph_mode stage_aware

  # Architecture sweep
  python maml_gnn_training_tsmc_parasitic.py \\
      --conv_hidden_dim 32 64 128 \\
      --num_conv_layers 2 3

  # Custom hyperparameters
  python maml_gnn_training_tsmc_parasitic.py --innerdiv 10 --meta_lr 0.0001 --K 5
"""
    )
...
def train_single_config(args):
    ...
    print(f"\n{'#'*80}")
    print(f"# MAML GNN Training - TSMC Parasitic Cap (12D features)")
    print(f"{'#'*80}")
    ...
    print(f"Node features: 12D (7 base + voltage/slew/load/temp/parasitic_cap)")
    ...
    train_file = f"train_{data_type}_{graph_mode}.pth"
    ...
    dataset = TSMCParasiticMAMLDataset(
    ...
    checkpoint_dir = "../../../pretrained_models/gnn_maml_tsmc_parasitic_checkpoints"
    final_model_dir = "../../../pretrained_models/gnn_maml_tsmc_parasitic_final"
    ...
    for idx, (conv_hidden_dim, num_conv_layers, fc_hidden_dim, num_fc_layers) in enumerate(arch_combinations, 1):
        ...
        gnn_maml = GNN_MAML_TSMCParasitic(
        ...
        for chunk in range(num_chunks):
            ...
            checkpoint_path = f"{checkpoint_dir}/gnn_maml_tsmc_parasitic_{data_type}_{graph_mode}_innerdiv{innerdiv}_meta{tasks_per_meta_batch}_iter{iterations_completed}_inner{inner_steps}{arch_suffix}.pth"
        ...
        pool_suffix = f"_pool{pooling_mode}" if pooling_mode != 'mean' else ""
        arch_suffix = f"_conv{conv_hidden_dim}x{num_conv_layers}_fc{fc_hidden_dim}x{num_fc_layers}{pool_suffix}"
        final_model_path = f"{final_model_dir}/gnn_maml_tsmc_parasitic_{data_type}_{graph_mode}_innerdiv{innerdiv}_meta{tasks_per_meta_batch}_iter{total_iterations}_inner{inner_steps}{arch_suffix}.pth"
