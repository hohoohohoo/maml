#!/bin/bash
# MAML GAT Training Sweep for TSMC Process Dataset (Unified 3D Format)
...
if [ $# -lt 1 ]; then
    ...
    echo ""
    echo "Example:"
    echo "  $0 json_configs/gat_maml_tsmc_process_sweep_config.json"
    echo "  $0 json_configs/gat_maml_tsmc_process_sweep_config.json --dry-run"
    echo ""
...
echo "=========================================="
echo "MAML GAT Training - TSMC Process (Unified)"
echo "=========================================="
...
if [ "$NO_COMMIT" = false ] && [ "$DRY_RUN" = false ]; then
    if git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        if git diff --cached --quiet; then
            ...
            COMMIT_MSG="Start MAML GAT TSMC Process sweep: $(basename $CONFIG_FILE)
...
# Build command - use GAT TSMC Process version
FULL_CMD="python -u maml_gat_training_tsmc_process.py $BASE_CONFIG $SWEEP_PARAMS $RESUME_CONFIG"
...
if [ "$DRY_RUN" = true ]; then
    ...
    echo "  2. Train GAT with multi-head attention"
    echo "  3. Train $TOTAL_COMBINATIONS architectures sequentially"
    echo "  4. Save all models to pretrained_models/gat_maml_tsmc_process_final/"
    echo ""
    echo "GAT-specific options:"
    echo "  - heads: Number of attention heads (default: 4)"
    echo "  - num_fc_layers: 2 or 3 (GAT supports only 2-3 FC layers)"
    echo ""
...
# Execute
echo "Starting GAT TSMC Process sweep..."
echo ""
echo "Command:"
echo "$FULL_CMD"
echo ""
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "GAT Sweep completed successfully"
else
    echo "GAT Sweep failed with exit code $EXIT_CODE"
fi
