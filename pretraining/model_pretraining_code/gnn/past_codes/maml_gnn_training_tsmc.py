#!/usr/bin/env python
"""
MAML GNN Training for TSMC Dataset - Lazy Loading Version
...
Key Features:
- Loads unified train file lazily (train_cell_stage_aware.pth)
...
class TSMCMAMLTaskDataset(Dataset):
    """
    PyTorch Dataset for TSMC MAML task data with true mmap loading.
    ...
    def __init__(self, train_path, graph_mode='stage_aware'):
        ...
        print(f"TSMCMAMLTaskDataset initialized (tensor format, mmap):")
...
class GNNTSMCLazyMAML:
    """
    GNN MAML with TSMC dataset and lazy loading.
    Uses TSMCMAMLTaskDataset for memory-efficient data access.
    """
    ...
    def __init__(self, model, inner_lr, meta_lr, K=5, inner_steps=1,
        ...
        Args:
            ...
            dataset: TSMCMAMLTaskDataset instance
        ...
        print(f"\nGNN TSMC Lazy MAML Configuration:")
    ...
    def main_loop_sequential(self, num_iterations):
        ...
        print(f"\nStarting TSMC Lazy MAML Training (Sequential)")
...
TSMC_TEMPERATURES = [25, 50, 75, 100]
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='MAML GNN Training for TSMC Dataset (Lazy Loading)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Single configuration using folder name
  python maml_gnn_training_tsmc.py --folder TSMC_FF_25 --graph_mode stage_aware

  # Using corner and temperature shorthand
  python maml_gnn_training_tsmc.py --corner FF --temperature 25

  # Architecture sweep
  python maml_gnn_training_tsmc.py --corner FF --temperature 0 \\
      --conv_hidden_dim 64 128 256 \\
      --num_conv_layers 2 3 4

  # Run all 15 configurations (3 corners x 5 temperatures)
  python maml_gnn_training_tsmc.py --run_all --graph_mode stage_aware
"""
    )
    ...
    parser.add_argument('--conv_hidden_dim', type=int, nargs='+', default=[64],
    ...
    parser.add_argument('--fc_hidden_dim', type=int, nargs='+', default=[512],
...
def train_single_config(folder_name, args):
    """Train MAML for a single TSMC folder"""
    import itertools
    ...
    print(f"\n{'#'*80}")
    print(f"# MAML GNN Training for TSMC Dataset (mmap Loading)")
    print(f"{'#'*80}")
    ...
    for idx, (conv_hidden_dim, num_conv_layers, fc_hidden_dim, num_fc_layers) in enumerate(arch_combinations, 1):
        ...
        print("Creating GNN TSMC MAML model...")

        gnn_maml = GNNTSMCLazyMAML(
        ...
        for chunk in range(num_chunks):
            ...
            # Save checkpoint
            checkpoint_dir = "../../../pretrained_models/gnn_maml_tsmc_checkpoints"
            ...
            checkpoint_path = f"{checkpoint_dir}/gnn_maml_{folder_name}_{data_type}_{graph_mode}_innerdiv{innerdiv}_meta{tasks_per_meta_batch}_iter{iterations_completed}_inner{inner_steps}{arch_suffix}{pooling_suffix}.pth"
        ...
        # Save final model
        final_model_dir = "../../../pretrained_models/gnn_maml_tsmc_final"
        ...
        final_model_path = f"{final_model_dir}/gnn_maml_{folder_name}_{data_type}_{graph_mode}_innerdiv{innerdiv}_meta{tasks_per_meta_batch}_iter{total_iterations}_inner{inner_steps}{arch_suffix}{pooling_suffix}.pth"
