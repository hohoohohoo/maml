#!/bin/bash
# MAML GNN Training Sweep for TSMC Parasitic Cap Dataset (Unified 3D Format)
# Uses pre-processed TSMC unified dataset with 12D node features (including R,C values)
#
# Node Features (12D):
#   - 7 base electrical features
#   - voltage, input_slew, output_load, temperature
#   - parasitic_cap (R,C values from LPE extraction)
#
...
if [ $# -lt 1 ]; then
    ...
    echo ""
    echo "Example:"
    echo "  $0 json_configs/gnn_maml_tsmc_parasitic_sweep_config.json"
    echo "  $0 json_configs/gnn_maml_tsmc_parasitic_sweep_config.json --dry-run"
    echo ""
    echo "Note: Requires TSMC unified dataset with parasitic cap to be pre-generated."
    echo "      Run data_preprocessing_gnn_process_cached_tsmc.sh first."
    echo ""
...
echo "=========================================="
echo "MAML GNN Training - TSMC Parasitic Cap (12D)"
echo "=========================================="
...
if [ "$NO_COMMIT" = false ] && [ "$DRY_RUN" = false ]; then
    if git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        if git diff --cached --quiet; then
            ...
            COMMIT_MSG="Start MAML GNN TSMC Parasitic sweep: $(basename $CONFIG_FILE)

Experiment Configuration:
- Config file: $CONFIG_FILE
- Timestamp: $TIMESTAMP
- Dataset: TSMC Parasitic Cap (12D node features: 7 base + voltage/slew/load/temp/parasitic_cap)
...
FULL_CMD="python -u maml_gnn_training_tsmc_parasitic.py $BASE_CONFIG $SWEEP_PARAMS"
...
if [ "$DRY_RUN" = true ]; then
    ...
    echo ""
    echo "This will:"
    echo "  1. Load TSMC Parasitic unified dataset (12D node features with R,C values)"
    echo "  2. Train $TOTAL_COMBINATIONS architectures sequentially"
    echo "  3. Save all models to pretrained_models/gnn_maml_tsmc_parasitic_final/"
    echo ""
...
# Execute
echo "Starting TSMC Parasitic sweep..."
echo ""
echo "Command:"
echo "$FULL_CMD"
echo ""
