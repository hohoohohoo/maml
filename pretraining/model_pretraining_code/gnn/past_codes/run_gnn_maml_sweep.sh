#!/bin/bash
# GNN MAML Training Sweep (Efficient Version)
...
if [ $# -lt 1 ]; then
    ...
    echo ""
    echo "Example:"
    echo "  $0 json_configs/gnn_maml_sweep_config.json"
    echo "  $0 json_configs/gnn_maml_sweep_config.json --dry-run"
    exit 1
fi
...
echo "=========================================="
echo "GNN MAML Training Sweep (Efficient)"
echo "=========================================="
...
if [ "$NO_COMMIT" = false ] && [ "$DRY_RUN" = false ]; then
    if git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        if git diff --cached --quiet; then
            ...
            COMMIT_MSG="Start GNN MAML efficient sweep: $(basename $CONFIG_FILE)
...
# Build command
FULL_CMD="python -u maml_gnn_training_cached.py $BASE_CONFIG $SWEEP_PARAMS"
