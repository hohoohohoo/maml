#!/usr/bin/env python
"""
Baseline GNN Training with Cached Topology - Mini-Batch Training
...
class GNN_Baseline_Pretraining:
    """
    ...
    def __init__(self, model, lr=2e-3, wd=5e-3,
        ...
        print(f"🎯 GNN Baseline Pretraining Configuration:")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Baseline GNN Training with Cached Topology (Mini-Batch)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python baseline_gnn_training_cached.py --process SRAM --corner FF --graph_mode stage_aware
  python baseline_gnn_training_cached.py --process LVT --corner TT --graph_mode full_graph --lr 0.001
"""
    )
...
def train_single_config(process, corner_type, args):
    ...
    print(f"\n🚀 Baseline GNN Training with Cached Topology (Mini-Batch)")
    ...
    for idx, (conv_hidden_dim, num_conv_layers, fc_hidden_dim, num_fc_layers) in enumerate(arch_combinations, 1):
        ...
        gnn_baseline = GNN_Baseline_Pretraining(
        ...
        print(f"{'='*80}")
        print(f"Total time: {(end_time - arch_start_time)/3600:.2f} hours")
        print(f"Final model saved: {final_model_path}")
        print(f"{'='*80}")

        print(f"\n🧹 Cleaning up memory after this architecture...")

        # Delete model-specific structures
...
def main():
    ...
    if args.run_all:
        ...
        all_processes = [ 'RVT', 'SLVT', 'LVT', 'SRAM']
