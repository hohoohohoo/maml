#!/bin/bash
# Baseline GNN Training Sweep with Unified Dataset (mmap Loading)
...
echo "=========================================="
echo "Baseline GNN Training - Unified Dataset"
echo "=========================================="
...
if [ "$NO_COMMIT" = false ] && [ "$DRY_RUN" = false ]; then
    if git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        if git diff --cached --quiet; then
            ...
            COMMIT_MSG="Start Baseline GNN unified sweep: $(basename $CONFIG_FILE)
...
FULL_CMD="python -u baseline_gnn_training_asap7.py $BASE_CONFIG $SWEEP_PARAMS"
