#!/bin/bash
# MAML GNN Training Sweep for TSMC Dataset
# Uses pre-processed TSMC dataset with train/test split
...
if [ $# -lt 1 ]; then
    ...
    echo ""
    echo "Example:"
    echo "  $0 json_configs/gnn_maml_tsmc_sweep_config.json"
    echo "  $0 json_configs/gnn_maml_tsmc_sweep_config.json --dry-run"
    echo ""
    echo "Note: Requires TSMC dataset to be pre-generated."
    echo "      Run build_gnn_dataset_tsmc_train_test_split.py first."
    exit 1
fi
...
echo "=========================================="
echo "MAML GNN Training - TSMC Dataset"
echo "=========================================="
...
if [ "$NO_COMMIT" = false ] && [ "$DRY_RUN" = false ]; then
    if git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        if git diff --cached --quiet; then
            ...
            COMMIT_MSG="Start MAML GNN TSMC sweep: $(basename $CONFIG_FILE)

Experiment Configuration:
- Config file: $CONFIG_FILE
- Timestamp: $TIMESTAMP
- Dataset: TSMC (11D node features)
...
# Build command - use TSMC version
FULL_CMD="python -u maml_gnn_training_tsmc.py $BASE_CONFIG $SWEEP_PARAMS"
...
if [ "$DRY_RUN" = true ]; then
    ...
    echo ""
    echo "This will:"
    echo "  1. Load TSMC dataset (11D node features)"
