#!/usr/bin/env python
"""
MAML GNN Training for TSMC Process Dataset - Unified 4-D V×T Format

Uses 2-D (V×T) datasets produced by tsmc/build_gnn_dataset_process_cached_tsmc_2d.py.
...
def find_pretrained_gnn_model(model_dir, data_type, graph_mode, innerdiv, meta,
    ...
    # Search pattern
    pattern = f"gnn_maml_tsmc_process_2d_{data_type}_{graph_mode}_innerdiv{innerdiv}_meta{meta}_iter*_inner{inner_steps}{arch_suffix}.pth"
...
class TSMCProcessMAMLDataset2D(Dataset):
    """
    ...
    def __init__(self, train_path, graph_mode='stage_aware', cache_path_override=None):
        ...
        print(f"TSMCProcessMAMLDataset2D initialized (4-D V×T format, mmap):")
...
DEFAULT_DATASET_DIR = "/home/tkdgn2907/Deepsets_test/MAML/Projects/dataset_all/gnn_dataset_2d"
...
def train_single_config(args):
    ...
    train_file = f"train_{data_type}_{graph_mode}{topology_suffix}{voltage_suffix}{norm_suffix}{temp_suffix}{slew_suffix}_2d.pth"
    ...
    dataset = TSMCProcessMAMLDataset2D(
    ...
    checkpoint_dir = f"../../../pretrained_models/gnn_maml_tsmc_process_2d_checkpoints{checkpoint_suffix}{voltage_suffix}{norm_suffix}{temp_suffix}{slew_suffix}"
    final_model_dir = f"../../../pretrained_models/gnn_maml_tsmc_process_2d_final{checkpoint_suffix}{voltage_suffix}{norm_suffix}{temp_suffix}{slew_suffix}"
    ...
    for idx, (conv_hidden_dim, num_conv_layers, fc_hidden_dim, num_fc_layers) in enumerate(arch_combinations, 1):
        ...
        for chunk in range(num_chunks):
            ...
            checkpoint_path = f"{checkpoint_dir}/gnn_maml_tsmc_process_2d_{data_type}_{graph_mode}_innerdiv{innerdiv}_meta{tasks_per_meta_batch}_iter{cumulative_iterations}_inner{inner_steps}{arch_suffix}.pth"
        ...
        pool_suffix = f"_pool{pooling_mode}" if pooling_mode != 'mean' else ""
        arch_suffix = f"_conv{conv_hidden_dim}x{num_conv_layers}_fc{fc_hidden_dim}x{num_fc_layers}{pool_suffix}"
        final_model_path = f"{final_model_dir}/gnn_maml_tsmc_process_2d_{data_type}_{graph_mode}_innerdiv{innerdiv}_meta{tasks_per_meta_batch}_iter{final_iteration}_inner{inner_steps}{arch_suffix}.pth"
