#!/usr/bin/env python3
"""
Parse GNN MAML sweep configuration JSON
Supports multi-parameter sweeps with cartesian product
Similar to MLP's parse_sweep_config.py
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python parse_gnn_maml_sweep_config.py <config.json>", file=sys.stderr)
        sys.exit(1)

    config_file = sys.argv[1]
    config = parse_config(config_file)

    experiment_name = config.get('experiment_name', 'gnn_sweep')
