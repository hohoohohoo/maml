...
def load_and_preprocess_data(data_type='cell', device='cuda'):
    ...
    test_data_input = torch.load(f"{data_dir2}/tsmc_intra_topology_train_input_{data_type}.pth")
    test_data_output_1 = torch.load(f"{data_dir2}/tsmc_intra_topology_train_output_{data_type}.pth")
...
def train_mlp1_model(num_iterations=100000, data_type='cell', learning_rate=1e-4, model_type='aadam', device='cuda'):
    ...
    checkpoint_dir = f'MLP_pretrained_model/checkpoints_intratopology_{data_type}_tsmc_{model_type}_{num_iterations}'
    mlp1_train_loss = mlp1.loop(checkpoint_dir=checkpoint_dir)

    # Save model using utility function
    model_save_path = f'MLP_pretrained_model/pretrained_mlp1_intratopology_{data_type}_tsmc_{model_type}_{num_iterations}.pth'
...
def main():
    ...
    parser = argparse.ArgumentParser(description='MLP Pretraining - TSMC Intra Topology')
    ...
    print(f"\n🚀 Starting MLP1 training on TSMC intra topology {data_type} dataset")
