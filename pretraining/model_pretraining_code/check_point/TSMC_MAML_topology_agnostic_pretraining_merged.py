...
def extract_iteration_from_filename(filepath, layer_length, inner):
    ...
    # 파일명 패턴: cell_innerdiv10_meta32_topology_agnostic_519traintask__full1DMAML_weights_3hidden_(40)_100000_inner1_upgraded_tsmc_merged.pth
    # 또는: cell_innerdiv10_meta32_topology_agnostic_519traintask__full1DMAML_weights_3hidden_40_100000_inner1_upgraded_tsmc_merged.pth
    pattern1 = rf'_\({layer_length}\)_(\d+)_inner{inner}_upgraded_tsmc_merged\.pth'
    pattern2 = rf'_{layer_length}_(\d+)_inner{inner}_upgraded_tsmc_merged\.pth'
...
def find_pretrained_model(model_dir, layer_length, inner, innerdiv=100, meta=32, data_type='cell'):
    """
    ...
    Args:
        ...
        meta: tasks per meta batch (default: 32)
        data_type: data type - cell/transition (default: 'cell')

    Returns:
        (filepath, iteration): 모델 파일 경로와 iteration 번호 튜플
    """
    # 괄호가 있는 경우와 없는 경우 모두 검색
    pattern1 = f"{data_type}_innerdiv{innerdiv}_meta{meta}_topology_agnostic_519traintask__full1DMAML_weights_3hidden_({layer_length})_*_inner{inner}_upgraded_tsmc_merged.pth"
    pattern2 = f"{data_type}_innerdiv{innerdiv}_meta{meta}_topology_agnostic_519traintask__full1DMAML_weights_3hidden_{layer_length}_*_inner{inner}_upgraded_tsmc_merged.pth"

    print(f"🔍 Searching for pretrained models...")
    models = []
    models.extend(glob.glob(os.path.join(model_dir, pattern1)))
    models.extend(glob.glob(os.path.join(model_dir, pattern2)))
...
def main():
    # 커맨드라인 인자 파싱
    parser = argparse.ArgumentParser(description='MAML Pretraining - Resume from checkpoint - MERGED DATASETS')
    ...
    print(f"\n🚀 Processing TSMC topology agnostic {data_type} dataset - MERGED")
    ...
    # 데이터 로드 및 전처리
    print("📊 Loading and preprocessing cell data from multiple sources...")

    # Dataset 1: topology_agnostic_data
    data_dir1 = "/home/tkdgn2907/Deepsets_test/MAML/Projects/dataset_all/dataset_tsmc_processed/topology_agnostic_data"
    print(f"📂 Loading dataset 1 from: {data_dir1}")
    test_data_input_1 = torch.load(f"{data_dir1}/tsmc_topology_agnostic_train_input_{data_type}.pth")
    test_data_output_1_1 = torch.load(f"{data_dir1}/tsmc_topology_agnostic_train_output_{data_type}.pth")
    print(f"   Dataset 1 shapes: Input {test_data_input_1.shape}, Output {test_data_output_1_1.shape}")

    # Dataset 2: topology_agnostic_data2
    data_dir2 = "/home/tkdgn2907/Deepsets_test/MAML/Projects/dataset_all/dataset_tsmc_processed/topology_agnostic_data2"
    print(f"📂 Loading dataset 2 from: {data_dir2}")
    test_data_input_2 = torch.load(f"{data_dir2}/tsmc_topology_agnostic_train_input_{data_type}.pth")
    test_data_output_1_2 = torch.load(f"{data_dir2}/tsmc_topology_agnostic_train_output_{data_type}.pth")
    print(f"   Dataset 2 shapes: Input {test_data_input_2.shape}, Output {test_data_output_1_2.shape}")

    # Merge datasets along the first dimension (samples)
    print(f"🔗 Merging datasets...")
    test_data_input = torch.cat([test_data_input_1, test_data_input_2], dim=0)
    test_data_output_1 = torch.cat([test_data_output_1_1, test_data_output_1_2], dim=0)

    print(f"✅ Merged data shapes: Input {test_data_input.shape}, Output {test_data_output_1.shape}")
    print(f"   Total samples: {test_data_input.shape[0]} (Dataset1: {test_data_input_1.shape[0]}, Dataset2: {test_data_input_2.shape[0]})")
    ...
    for chunk in range(1, num_chunks + 1):
        ...
        checkpoint_path = f"{checkpoint_dir}/{data_type}_innerdiv{innerdiv}_meta{meta}_topology_agnostic_519traintask__full1DMAML_weights_3hidden_({layer_length})_{current_iteration}_inner{inner}_upgraded_tsmc_adding_NAND_NOR.pth"
    ...
    final_model_path = f"{pretrained_models_dir}/{data_type}_innerdiv{innerdiv}_meta{meta}_topology_agnostic_519traintask_full1DMAML_weights_3hidden_({layer_length})_{final_iteration}_inner{inner}_upgraded_tsmc_adding_NAND_NOR.pth"
