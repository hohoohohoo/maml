...
def main():
    # 커맨드라인 인자 파싱
    parser = argparse.ArgumentParser(description='MAML Pretraining - Topology Agnostic Cell')
    ...
    data_dir2 = "/home/tkdgn2907/Deepsets_test/MAML/Projects/datset_all/dataset_tsmc_processed/topology_agnostic_data"
    ...
    if args.resume:
        ...
        # 자동으로 최신 모델 찾아서 로드
        model_path, iteration = find_pretrained_model(pretrained_models_dir, layer_length, inner, innerdiv, meta, data_type, tech='tsmc')
        if model_path:
    ...
    for chunk in range(1, num_chunks + 1):
        ...
        checkpoint_path = f"{checkpoint_dir}/{data_type}_innerdiv{innerdiv}_meta{meta}_topology_agnostic_519traintask_full1DMAML_weights_3hidden_({layer_length})_{current_iteration}_inner{inner}_upgraded_tsmc.pth"
    ...
    final_model_path = f"{pretrained_models_dir}/{data_type}_innerdiv{innerdiv}_meta{meta}_topology_agnostic_519traintask_full1DMAML_weights_3hidden_({layer_length})_{final_iteration}_inner{inner}_upgraded_tsmc.pth"
