...
def extract_iteration_from_filename(filepath, layer_length, inner):
    ...
    # 파일명 패턴: cell_innerdiv100_meta32_intra_topology_519traintask_full1DMAML_weights_3hidden_(40)_100000_inner1_upgraded_tsmc.pth
    # 또는: cell_innerdiv100_meta32_intra_topology_519traintask_full1DMAML_weights_3hidden_40_100000_inner1_upgraded_tsmc.pth
    pattern1 = rf'_\({layer_length}\)_(\d+)_inner{inner}_upgraded_tsmc\.pth'
    pattern2 = rf'_{layer_length}_(\d+)_inner{inner}_upgraded_tsmc\.pth'
...
def find_pretrained_model(model_dir, layer_length, inner, innerdiv=100, meta=32, data_type='cell'):
    ...
    pattern1 = f"{data_type}_innerdiv{innerdiv}_meta{meta}_intra_topology_519traintask_full1DMAML_weights_3hidden_({layer_length})_*_inner{inner}_upgraded_tsmc_without_INV.pth"
    pattern2 = f"{data_type}_innerdiv{innerdiv}_meta{meta}_intra_topology_519traintask_full1DMAML_weights_3hidden_{layer_length}_*_inner{inner}_upgraded_tsmc_without_INV.pth"
...
def main():
    ...
    print(f"\n🚀 Processing TSMC intra topology {data_type} dataset")
    ...
    # 데이터 로드 및 전처리
    print("📊 Loading and preprocessing invbuf data...")
    
    # Merged invbuf data 경로
    data_dir2 = "/home/tkdgn2907/Deepsets_test/MAML/Projects/dataset_all/dataset_tsmc_processed/intra_topology_data_without_INV"
    test_data_input = torch.load(f"{data_dir2}/tsmc_intra_topology_train_input_{data_type}.pth")     # [total_samples, 61, 9]
    test_data_output_1 = torch.load(f"{data_dir2}/tsmc_intra_topology_train_output_{data_type}.pth")  # [total_samples, 61, 1]

    #print(test_data_input.size(),test_data_output_1.size())
    ...
    for chunk in range(1, num_chunks + 1):
        ...
        checkpoint_path = f"{checkpoint_dir}/{data_type}_innerdiv{innerdiv}_meta{meta}_intra_topology_519traintask_full1DMAML_weights_3hidden_({layer_length})_{current_iteration}_inner{inner}_upgraded_tsmc_without_INV.pth"
    ...
    final_model_path = f"{pretrained_models_dir}/{data_type}_innerdiv{innerdiv}_meta{meta}_intra_topology_519traintask_full1DMAML_weights_3hidden_({layer_length})_{final_iteration}_inner{inner}_upgraded_tsmc_without_INV.pth"
