...
def load_and_preprocess_data(data_type='cell', device='cuda'):
    ...
    data_dir2 = "/home/tkdgn2907/Deepsets_test/MAML/Projects/dataset_all/dataset_tsmc_processed/topology_agnostic_data"
    test_data_input = torch.load(f"{data_dir2}/tsmc_topology_agnostic_train_input_{data_type}.pth")
    test_data_output_1 = torch.load(f"{data_dir2}/tsmc_topology_agnostic_train_output_{data_type}.pth")
...
def train_mlp1_model(num_iterations=100000, data_type='cell', learning_rate=1e-4, model_type='aadam', device='cuda'):
    ...
    checkpoint_dir = f'MLP_pretrained_model/checkpoints_topology_agnostic_{data_type}_tsmc_{model_type}_{num_iterations}'
    mlp1_train_loss = mlp1.loop(checkpoint_dir=checkpoint_dir)

    # Save model using utility function
    model_save_path = f'MLP_pretrained_model/pretrained_mlp1_topology_agnostic_{data_type}_tsmc_{model_type}_{num_iterations}.pth'
...
def main():
    ...
    parser = argparse.ArgumentParser(description='MLP Pretraining - TSMC Topology Agnostic')
    ...
    print(f"\n🚀 Starting MLP1 training on TSMC topology agnostic {data_type} dataset")
