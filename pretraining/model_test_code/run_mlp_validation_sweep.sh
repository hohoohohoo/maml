...
if [ $# -lt 1 ]; then
    ...
    echo ""
    echo "Example:"
    echo "  $0 json_configs/mlp_validation_sweep.json"
    echo "  $0 json_configs/mlp_validation_sweep.json --dry-run"
    echo "  $0 json_configs/mlp_validation_sweep.json --no-commit"
    exit 1
fi
...
- Timestamp: $TIMESTAMP
- Script: run_mlp_validation_sweep.sh
...
BASE_CMD="nohup python MLP_topology_validation.py"
BASE_ARGS=""
