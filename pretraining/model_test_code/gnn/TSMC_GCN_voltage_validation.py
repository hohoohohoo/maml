...
def run_validation(args, corner, temperature, device):
    ...
    if args.model_path is None:
        ...
        if args.model_type == 'baseline':
            ...
            model_filename = f"gnn_baseline_{folder_name}_{args.data_type}_{args.graph_mode}_iter{args.num_iterations}{arch_suffix}.pth"
        else:  # maml
            model_dir = "../../../pretrained_models/gnn_maml_tsmc_final"
            model_filename = f"gnn_maml_{folder_name}_{args.data_type}_{args.graph_mode}_innerdiv{args.innerdiv}_meta{args.tasks_per_meta_batch}_iter{args.num_iterations}_inner{args.inner_steps}{arch_suffix}.pth"
    ...
    if args.save_results:
        ...
        if args.model_type == 'baseline':
            base_name = f"{args.output_prefix}_{folder_name}_{args.data_type}_{args.graph_mode}_{args.mode}_{args.model_type}_iter{args.num_iterations}{arch_suffix}{filter_suffix}"
        else:
            base_name = f"{args.output_prefix}_{folder_name}_{args.data_type}_{args.graph_mode}_{args.mode}_{args.model_type}_innerdiv{args.innerdiv}_meta{args.tasks_per_meta_batch}_iter{args.num_iterations}_inner{args.inner_steps}{arch_suffix}{filter_suffix}"
...
Examples:
  ...
  python TSMC_GCN_voltage_validation_unified.py --run_all --model_type baseline --gpu 5
    ...
    if args.run_all:
        ...
        if all_results:
            print(f"{'Corner':<8} {'Temp':<8} {'Tasks':<8} {'NRMSE%':<10} {'MAPE%':<10} {'MAE(ns)':<12}")
            print("-" * 60)
