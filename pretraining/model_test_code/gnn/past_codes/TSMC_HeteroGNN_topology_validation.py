#!/usr/bin/env python
"""
TSMC Heterogeneous GNN Topology Validation Script

This script validates Heterogeneous GNN models trained with MAML for TSMC topology experiments:
- topology_agnostic: Test on unseen cell topologies
- intra_topology: Test on seen cell topologies with unseen conditions

Uses type-specific transformations for different node types (Power, Port, NMOS, PMOS).

Usage:
  python TSMC_HeteroGNN_topology_validation.py --experiment intra_topology --model_type maml --gpu 0
  python TSMC_HeteroGNN_topology_validation.py --experiment topology_agnostic --model_type maml --gpu 1
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='TSMC Heterogeneous GNN Topology Validation',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python TSMC_HeteroGNN_topology_validation.py --experiment intra_topology --model_type maml --gpu 0
  python TSMC_HeteroGNN_topology_validation.py --experiment topology_agnostic --model_type maml --gpu 1
  python TSMC_HeteroGNN_topology_validation.py --experiment intra_topology --model_type maml --mode interpolation
        """
    )
    ...
    parser.add_argument('--num_conv_layers', type=int, default=2,
                        help='Number of HeteroGNN convolutional layers (default: 2)')
    ...
    parser.add_argument('--conv_type', type=str, default='gcn',
                        choices=['gcn', 'gat'],
                        help='Convolution type: gcn or gat (default: gcn)')
    ...
    parser.add_argument('--output_prefix', type=str, default='TSMC_HeteroGNN',
    ...
    print(f"\n{'='*80}")
    print(f"TSMC Heterogeneous GNN Topology Validation")
    print(f"{'='*80}")
    ...
    print(f"Node types: {args.num_node_types}")
    ...
    # Load model
    print(f"\nLoading {args.model_type.upper()} Heterogeneous GNN model...")

    if args.model_path is None:
        # Add pooling suffix only if not 'mean' (default)
        pool_suffix = f"_pool{args.pooling}" if args.pooling != 'mean' else ""
        conv_type_suffix = f"_{args.conv_type}" if args.conv_type != 'gcn' else ""
        arch_suffix = f"_conv{args.conv_hidden_dim}x{args.num_conv_layers}_fc{args.fc_hidden_dim}x{args.num_fc_layers}{pool_suffix}{conv_type_suffix}"

        # Extract gatectrl from cache_path (gatectrl only affects adjacency, not train data)
        cache_suffix = "_gatectrl" if args.cache_path and "_gatectrl" in args.cache_path else ""
        ...
        if args.model_type == 'baseline':
            model_dir = f"../../../pretrained_models/hetero_gnn_baseline_tsmc_process_final{checkpoint_suffix}{voltage_suffix}{norm_suffix}{temp_suffix}{slew_suffix}{conv_type_suffix}"
            model_filename = f"hetero_gnn_baseline_tsmc_process_{args.data_type}_{args.graph_mode}_iter{args.num_iterations}{arch_suffix}.pth"
        else:
            model_dir = f"../../../pretrained_models/hetero_gnn_maml_tsmc_process_checkpoints{checkpoint_suffix}{voltage_suffix}{norm_suffix}{temp_suffix}{slew_suffix}{conv_type_suffix}"
            model_filename = f"hetero_gnn_maml_tsmc_process_{args.data_type}_{args.graph_mode}_innerdiv{args.innerdiv}_meta{args.tasks_per_meta_batch}_iter{args.num_iterations}_inner{args.inner_steps}{arch_suffix}.pth"
    ...
    conv_type = config.get('conv_type', args.conv_type)
    num_node_types = config.get('num_node_types', args.num_node_types)
    heads = config.get('heads', args.heads)

    # Get node_features from checkpoint weight shape
    # For HeteroGNN, the input linear is in input_linears
    node_features = checkpoint['model_state_dict']['input_linears.0.weight'].shape[1]
    ...
    print(f"Architecture: conv={conv_hidden_dim}x{num_conv_layers}, fc={fc_hidden_dim}x{num_fc_layers}, pooling={pooling_mode}, conv_type={conv_type}")
    ...
    if args.save_results and all_results:
        ...
        for result in all_results:
            ...
            if args.model_type == 'baseline':
                base_name = f"{args.output_prefix}_{args.experiment}_{cell_name}_{args.data_type}_{args.graph_mode}_{args.mode}_{args.model_type}_iter{args.num_iterations}{arch_suffix}{filter_suffix}{adapt_suffix}"
            else:
                base_name = f"{args.output_prefix}_{args.experiment}_{cell_name}_{args.data_type}_{args.graph_mode}_{args.mode}_{args.model_type}_innerdiv{args.innerdiv}_meta{args.tasks_per_meta_batch}_iter{args.num_iterations}_inner{args.inner_steps}{arch_suffix}{filter_suffix}{adapt_suffix}"
