#!/usr/bin/env python
"""
TSMC GCN Validation Script (Unified Dataset Version)
...
- Same validation logic as ASAP7 version but for TSMC dataset

TSMC Dataset Structure:
- 3 corners: TT, FF, SS
- 5 temperatures: 0, 25, 50, 75, 100
- 15 total configurations: TSMC_{corner}_{temperature}
- Each configuration has 61 voltage conditions (libs)

Usage:
    # Single configuration
    python TSMC_GCN_voltage_validation_unified.py --corner FF --temperature 0 --model_type baseline

    # Run all 15 configurations
    python TSMC_GCN_voltage_validation_unified.py --run_all --model_type maml
"""
...
class UnifiedTestDataset:
    ...
    """
    def __init__(self, unified_test_path, graph_mode='stage_aware'):
    ...
    def _load_data(self):
        ...
        # Print continuity stats if available (pre-computed in split_gnn_dataset_tsmc.py)
...
def load_unified_test_data(corner, temperature, data_type='cell', graph_mode='stage_aware'):
    """
    Load unified test data for a corner-temperature combination.

    Returns:
        UnifiedTestDataset object
    """
    folder_name = get_folder_name(corner, temperature)
    print(f"   Loading unified test data for: {folder_name}")
    print(f"   Data type: {data_type}")
    print(f"   Graph mode: {graph_mode}")

    # Base path for TSMC unified datasets (matches split_gnn_dataset_tsmc.py output)
    base_path = "/home/tkdgn2907/Deepsets_test/MAML/Projects/dataset_all/dataset_TSMC_GNN"

    # Build path: TSMC_{corner}_{temp}/train_test_split/test_{data_type}_{graph_mode}.pth
    filename = f"test_{data_type}_{graph_mode}.pth"
    unified_path = os.path.join(base_path, folder_name, "train_test_split", filename)

    if not os.path.exists(unified_path):
        raise FileNotFoundError(f"Unified test data not found: {unified_path}\n"
                               f"Run split_gnn_dataset_tsmc.py --corner {corner} --temperature {temperature} first.")

    dataset = UnifiedTestDataset(unified_path, graph_mode)
    return dataset


def run_validation(args, corner, temperature, device):
    """
    Run validation for a single corner-temperature combination.
    """
    folder_name = get_folder_name(corner, temperature)

    print(f"\n{'='*80}")
    print(f"   Running validation for: {folder_name}")
    print(f"{'='*80}")
    ...
    try:
        test_dataset = load_unified_test_data(
            corner, temperature, args.data_type, args.graph_mode
        )
    except Exception as e:
    ...
    # Get norm_stats from dataset (pre-computed in split_gnn_dataset_tsmc.py)
    ...
    if args.model_path is None:
        ...
        pooling_suffix = f"_pool{args.pooling}" if args.pooling != 'mean' else ""

        # Auto-determine model path for TSMC
        if args.model_type == 'baseline':
            model_dir = "../../../pretrained_models/gnn_baseline_tsmc_final"
            model_filename = f"gnn_baseline_{folder_name}_{args.data_type}_{args.graph_mode}_iter{args.num_iterations}{arch_suffix}{pooling_suffix}.pth"
        else:  # maml
            model_dir = "../../../pretrained_models/gnn_maml_tsmc_final"
            model_filename = f"gnn_maml_{folder_name}_{args.data_type}_{args.graph_mode}_innerdiv{args.innerdiv}_meta{args.tasks_per_meta_batch}_iter{args.num_iterations}_inner{args.inner_steps}{arch_suffix}{pooling_suffix}.pth"
    ...
    # Get pooling from checkpoint config or use command line args
    pooling_mode = config.get('pooling', args.pooling)
    output_node_idx = config.get('output_node_idx', args.output_node_idx)
    ...
    if args.filter_continuous:
        continuous_indices, _ = filter_continuous_tasks(
            ...
            if y_std > 0:
                ...
                if edge_index.numel() > 0:
                    ...
                    if max_idx >= num_nodes:
                        raise ValueError(f"Edge index out of bounds for cell {cell_name}: "
                                       ...
                                       f"adj_shape={adjacency_matrix.shape}")
                ...
                for idx in range(left_bound, right_bound):
                    ...
                    if edge_index.numel() > 0:
                        max_idx = edge_index.max().item()
                        if max_idx >= num_nodes:
                            raise ValueError(f"Edge index out of bounds: max_idx={max_idx}, num_nodes={num_nodes}")
                ...
                if abs(max_val - min_val) > 0:
                    ...
                    if i % 100 == 0 and i > 0:
                        ...
                        print(f"     Current avg RMSE: {1000*sum(total_rmse)/len(total_rmse):.3f}ps - Tasks: {len(total_rmse)}")
        except Exception as e:
            if i < 10:
    ...
    if len(total_nrmse) == 0:
        print(f"   No valid tasks processed for {folder_name}")
        return None
    ...
    print(f"   RMSE (Total/Left/Right/Inter): [{results['rmse_total']:.6f}, "
          f"{results['rmse_left']:.6f}, "
          f"{results['rmse_right']:.6f}, "
          f"{results['rmse_inter']:.6f}]")
    ...
    if args.save_results:
        ...
        if args.model_type == 'baseline':
            base_name = f"{args.output_prefix}_{folder_name}_{args.data_type}_{args.graph_mode}_{args.mode}_{args.model_type}_iter{args.num_iterations}{arch_suffix}{pooling_suffix}{filter_suffix}"
        else:
            base_name = f"{args.output_prefix}_{folder_name}_{args.data_type}_{args.graph_mode}_{args.mode}_{args.model_type}_innerdiv{args.innerdiv}_meta{args.tasks_per_meta_batch}_iter{args.num_iterations}_inner{args.inner_steps}{arch_suffix}{pooling_suffix}{filter_suffix}"
...
def main():
    parser = argparse.ArgumentParser(
        description='TSMC GCN Validation (Unified Dataset Version)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Single configuration
  python TSMC_GCN_voltage_validation_unified.py --corner FF --temperature 0 --model_type baseline --gpu 5
  python TSMC_GCN_voltage_validation_unified.py --corner TT --temperature 25 --model_type maml --mode interpolation

  # Using folder name directly
  python TSMC_GCN_voltage_validation_unified.py --folder TSMC_FF_0 --model_type baseline

  # Run all 15 configurations (3 corners x 5 temperatures)
  python TSMC_GCN_voltage_validation_unified.py --run_all --model_type baseline --gpu 5
        """
    )
    ...
    parser.add_argument('--folder', type=str, default=None,
                       help='Folder name directly (e.g., TSMC_FF_0)')
    parser.add_argument('--run_all', action='store_true',
                       help='Run validation for all 15 corner-temperature combinations')
    ...
    parser.add_argument('--output_node_idx', type=int, default=2,
                       help='Fallback index of output node for output pooling (default: 2)')
    ...
    parser.add_argument('--num_iterations', type=int, default=30000,
                       help='Pretraining iterations (default: 30000)')

    # Results saving
    parser.add_argument('--save_results', action='store_true',
                       help='Save prediction results to .npy files')
    parser.add_argument('--output_prefix', type=str, default='TSMC_GCN_unified',
                       help='Prefix for output files (default: TSMC_GCN_unified)')
    ...
    # Parse folder name if provided
    if args.folder is not None:
        import re
        match = re.match(r'TSMC_([A-Z]+)_(\d+)', args.folder)
        if match:
            args.corner = match.group(1)
            args.temperature = int(match.group(2))
        else:
            parser.error(f"Invalid folder name format: {args.folder}. Expected TSMC_<CORNER>_<TEMP>")

    # Validate arguments
    if not args.run_all and (args.corner is None or args.temperature is None):
        parser.error("--corner and --temperature are required unless --run_all or --folder is specified")
    ...
    if args.run_all:
        print(f"\n{'#'*80}")
        print(f"# Running ALL 15 corner-temperature combinations")
        print(f"# Model type: {args.model_type}")
        print(f"# Mode: {args.mode}")
        ...
        for corner in TSMC_CORNERS:
            for temp in TSMC_TEMPERATURES:
                result = run_validation(args, corner, temp, device)
        ...
        # Print summary table
        print(f"\n{'='*80}")
        print(f"   SUMMARY: All Corner-Temperature Combinations")
        print(f"{'='*80}")
        print(f"   Successful: {successful}/15, Failed: {failed}/15")
        print()

        if all_results:
            print(f"{'Corner':<8} {'Temp':<8} {'Tasks':<8} {'NRMSE%':<10} {'MAPE%':<10} {'RMSE(ns)':<12}")
            print("-" * 60)

            for result in all_results:
                print(f"{result['corner']:<8} {result['temperature']:<8} {result['num_valid_tasks']:<8} "
                      f"{result['nrmse_total']:<10.2f} {result['mape_total']:<10.2f} {result['rmse_total']*1e9:<12.4f}")
            ...
            print("-" * 60)
            print(f"{'AVERAGE':<8} {'':<8} {'':<8} {avg_nrmse:<10.2f} {avg_mape:<10.2f} {avg_rmse*1e9:<12.4f}")

        return 0 if failed == 0 else 1

    else:
        result = run_validation(args, args.corner, args.temperature, device)
