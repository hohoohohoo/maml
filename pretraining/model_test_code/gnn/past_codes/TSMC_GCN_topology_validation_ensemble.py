#!/usr/bin/env python
"""
TSMC GCN Topology Validation Script — ENSEMBLE + variance-adaptive conformal

Derived from TSMC_GCN_topology_validation.py.  All single-model behaviour is
preserved; the new functionality is gated behind --model_paths (a list of
checkpoints):

  - Each of the K checkpoints runs through its OWN grad+move + selective-Adam
    adaptation per task, producing K predicted curves of shape [61].
  - The ensemble MEAN is reported as the point estimate; the ensemble STD per
    query point is the local uncertainty sigma_local(x).
  - The post-hoc conformal block (--conformal_alpha) now operates in *local-
    sigma units*: delta_norm = (1-alpha)-quantile of (act - mean) / sigma_local
    over the calibration tasks, and at eval time each query point is shifted by
    delta_norm * sigma_local(x).  Confident predictions get small shifts,
    uncertain ones get larger shifts — breaking the constant-shift Pareto.

Backwards compatible: --model_path (singular) still works for K=1.

Usage:
  # Single-model (legacy):
  python TSMC_GCN_topology_validation_ensemble.py --experiment topology_agnostic \\
      --model_type maml --model_path <ckpt> --gpu 0

  # K=3 ensemble + variance-adaptive conformal at alpha=0.05:
  python TSMC_GCN_topology_validation_ensemble.py --experiment topology_agnostic \\
      --model_type maml \\
      --model_paths <ckpt_iter200k> <ckpt_iter300k> <ckpt_iter400k> \\
      --conformal_alpha 0.05 --conformal_split 0.5 --gpu 0
"""
...
def run_cell_validation(args, cell_name, topology_cache, norm_stats, models, device):
    """
    Run validation for a single cell.

    `models` is always a list of one or more nn.Module instances.  For K=1 the
    behaviour matches the original single-model script; for K>1 the script runs
    each model through its own grad+move + selective-Adam adaptation and stores
    the ensemble mean (point estimate) and ensemble std (per-point uncertainty).
    """
    K = len(models)
    # Convenience handle for the 'reference' model used only for the support-set
    # grad/move computation that pre-conditions adaptation.  Using model[0]
    # consistently means every task's grad/move is anchored to the same
    # reference, while each ensemble member then runs its own adaptation.
    ref_model = models[0]
    ...
    all_task_scales = []
    # Per-task per-point ensemble std (K=1 ⇒ all zeros). Used by the conformal
    # block for variance-adaptive shifts when K>=2 (smaller shift where the
    # ensemble agrees, larger shift where it disagrees).
    all_pred_stds = []
    ...
    for i, randomtask in enumerate(test_indices_list):
        ...
        try:
            ...
            if y_std > 0:
                ...
                # All grad/move anchors come from the reference (first) model so
                # every ensemble member shares the same support-set scaling and
                # adaptation target — only their parameters differ. This isolates
                # *parameter-space* uncertainty from any scaling jitter.
                with torch.no_grad():
                    center = ref_model(center_batch).item()
                ...
                if abs(max_val - min_val) > 0:
                    ...
                    # Per-model adaptation: each ensemble member runs its own
                    # grad+move + selective-Adam pipeline, producing one [61]
                    # curve. Stack to [K, 61] for downstream mean/std.
                    member_preds = []
                    member_acts  = None
                    any_adam_used = False
                    ...
                    for m_ in models:
                        ...
                        member_preds.append(np.asarray(predictions, dtype=np.float64))
                        member_acts = np.asarray(actual_values, dtype=np.float64)
                        any_adam_used = any_adam_used or bool(adam_used)
                        last_total_loss, last_mape_loss, last_rmse_loss = (
                            total_loss1, mape_loss1, rmse_loss)

                    # Stack ensemble: [K, 61]
                    member_preds_arr = np.stack(member_preds, axis=0)
                    pred_mean = member_preds_arr.mean(axis=0)                 # [61]
                    pred_std  = member_preds_arr.std(axis=0, ddof=0)          # [61]

                    # For K=1 std is zero — we still record it; the conformal
                    # block falls back gracefully to a constant raw shift in
                    # that case.
                    predictions    = pred_mean.tolist()
                    actual_values  = member_acts.tolist()
                    ...
                    if task_sign_flip < 0:
                        ...
                        pred_std      = pred_std  # std unchanged under sign flip
                    ...
                    # Per-task per-point ensemble std (local uncertainty).
                    all_pred_stds.append(pred_std.tolist())

                    # Reattach metric variable names expected downstream by the
                    # NRMSE / MAPE summary code so it sees ensemble-mean error
                    # rather than the last member's error.
                    err_mean = (member_acts - pred_mean)
                    total_loss1 = float((err_mean ** 2).mean())
                    mape_loss1  = float((np.abs(err_mean) / np.maximum(np.abs(member_acts), 1e-12)).mean())
                    rmse_loss   = float(np.sqrt((err_mean ** 2).mean()))
    ...
    # ------------------------------------------------------------------
    # Post-hoc per-cell conformal calibration.  Two modes — selected by
    # whether per-prediction ensemble std is available:
    #
    #  (a) K=1 (legacy): residuals normalized by per-task scale
    #      (y_std_support × grad). delta_norm is a unit-less number on the
    #      same scale as safe_eps. Eval-time shift = delta_norm × scale_task
    #      (constant across the 61 query points of one task).
    #
    #  (b) K>=2 (ensemble, variance-adaptive): residuals normalized by
    #      per-point ensemble std sigma_local(x) of the K members. Eval-time
    #      shift = delta_norm × sigma_local(x) — *different per query point*.
    #      Confident points (low sigma) move very little; uncertain points
    #      (high sigma) get the bulk of the safety margin. This is what
    #      enables breaking the constant-shift Pareto. A tiny sigma_floor
    #      = epsilon × scale_task keeps shifts non-degenerate where the
    #      ensemble fully agrees.
    #
    # Operates orthogonally to safe_eps / asym_alpha / pinball_tau (which
    # affect *adaptation*; this acts only on the final predictions). Under
    # task exchangeability the eval under-prediction rate <= alpha.
    # ------------------------------------------------------------------
    if args.conformal_alpha is not None and len(all_predictions) >= 2 * args.total_points:
        ...
        std_arr   = (np.array(all_pred_stds[:n_tasks_done], dtype=np.float64)
                     if len(all_pred_stds) >= n_tasks_done else np.zeros((n_tasks_done, gp)))
        ...
        pred_cal, act_cal, scale_cal, std_cal = pred_arr[:n_cal], act_arr[:n_cal], scale_arr[:n_cal], std_arr[:n_cal]
        pred_ev,  act_ev,  scale_ev,  std_ev  = pred_arr[n_cal:], act_arr[n_cal:], scale_arr[n_cal:], std_arr[n_cal:]

        use_ensemble_sigma = (std_arr.size > 0) and (std_arr.max() > 0) and (args.conformal_mode == 'ensemble')
        if use_ensemble_sigma:
            # Variance-adaptive (ensemble) conformal — Plan B from above.
            # Floor sigma at conformal_sigma_floor × scale_task so points where
            # the ensemble fully agrees still get a (small) safety shift.
            sigma_floor_cal = args.conformal_sigma_floor * scale_cal[:, None]
            sigma_floor_ev  = args.conformal_sigma_floor * scale_ev[:, None]
            sigma_cal_eff   = np.maximum(std_cal, sigma_floor_cal)
            sigma_ev_eff    = np.maximum(std_ev,  sigma_floor_ev)

            residuals_norm_cal = ((act_cal - pred_cal) / sigma_cal_eff).ravel()
            delta_norm = float(np.quantile(residuals_norm_cal, 1.0 - args.conformal_alpha))

            per_point_shift = delta_norm * sigma_ev_eff        # [n_eval, gp]
            raw_shift_mean = float(per_point_shift.mean())
            sigma_mean = float(sigma_ev_eff.mean())
            sigma_min  = float(sigma_ev_eff.min())
            sigma_max  = float(sigma_ev_eff.max())
            mode_tag = f"ENSEMBLE  K={K}  sigma_floor={args.conformal_sigma_floor:g}"
        else:
            # K=1 path (legacy): per-task constant shift in safe_eps units.
            residuals_norm_cal = ((act_cal - pred_cal) / scale_cal[:, None]).ravel()
            delta_norm = float(np.quantile(residuals_norm_cal, 1.0 - args.conformal_alpha))
            raw_shift_per_task = delta_norm * scale_ev          # [n_eval]
            per_point_shift = np.broadcast_to(raw_shift_per_task[:, None], pred_ev.shape)
            raw_shift_mean = float(per_point_shift.mean())
            sigma_mean = sigma_min = sigma_max = 0.0
            mode_tag = "SCALAR (K=1)"

        cal_under = float((residuals_norm_cal > 0).mean())
        ev_under_before = float(((act_ev - pred_ev) > 0).mean())
        pred_ev_shifted = pred_ev + per_point_shift
        ...
        print(f"   [conformal alpha={args.conformal_alpha:g}, split={args.conformal_split:g}, "
              ...
              f"raw_shift_mean={raw_shift_mean:+.6e}  sigma_eff[min/mean/max]="
              f"{sigma_min:.2e}/{sigma_mean:.2e}/{sigma_max:.2e}  "
...
Examples:
  ...
  python TSMC_GCN_topology_validation.py --experiment intra_topology --model_type maml --mode interpolation
    ...
    parser.add_argument('--conformal_mode', type=str, default='ensemble',
                        choices=['ensemble', 'scalar'],
                        help=('When K>=2 ensemble members are loaded, "ensemble" mode normalizes '
                              'residuals by per-point ensemble std sigma_local(x) (variance-adaptive '
                              'shift) while "scalar" mode falls back to the legacy per-task-constant '
                              'shift = delta_norm * scale_task. K=1 always uses scalar. Default: ensemble'))
    parser.add_argument('--conformal_sigma_floor', type=float, default=0.005,
                        help=('Minimum effective sigma_local floor for ensemble conformal, expressed '
                              'as a fraction of scale_task (= y_std_support * grad). Prevents '
                              'degenerate near-zero shifts at points where the ensemble fully '
                              'agrees. Default: 0.005 (= 0.5%% of safe_eps unit).'))
    parser.add_argument('--model_paths', type=str, nargs='+', default=None,
                        help=('List of checkpoint paths for ensemble inference (K>=2). Each member '
                              'runs its own grad+move + selective-Adam adaptation per task; the '
                              'reported point estimate is the ensemble mean, and per-point ensemble '
                              'std seeds the variance-adaptive conformal shift. Overrides --model_path. '
                              'Pass space-separated absolute paths. Example: '
                              '--model_paths ckpt_iter200k.pth ckpt_iter300k.pth ckpt_iter400k.pth'))
    ...
    # ------------------------------------------------------------------
    # Resolve the list of checkpoint paths to load.  --model_paths wins;
    # otherwise we keep the legacy --model_path / auto-resolved single
    # checkpoint behaviour and treat it as K=1.
    # ------------------------------------------------------------------
    if args.model_paths:
        resolved_model_paths = list(args.model_paths)
        ...
        resolved_model_paths = [os.path.join(model_dir, model_filename)]
    else:
        resolved_model_paths = [args.model_path]
    ...
    resolved_model_paths = [
        p if os.path.isabs(p) else os.path.normpath(os.path.join(here, p))
        for p in resolved_model_paths
    ]
    missing = [p for p in resolved_model_paths if not os.path.exists(p)]
    if missing:
        for p in missing:
            print(f"Model not found: {p}")
        print("Please provide --model_path / --model_paths or ensure the model exists")
        return 1

    K = len(resolved_model_paths)
    print(f"Ensemble size K = {K}")
    for i, p in enumerate(resolved_model_paths):
        print(f"  member [{i}]: {p}")

    # Load primary checkpoint to retrieve norm_stats / config / arch dims.
    # Subsequent members are loaded with the same architecture; if they were
    # trained with different node_features / conv-dim that's a config error
    # in the sweep and will surface at state_dict load.
    checkpoint = torch.load(resolved_model_paths[0], weights_only=False, map_location=device)
    ...
    if checkpoint_norm_stats is not None:
        ...
        print("Using norm_stats from checkpoint[0]")
    ...
    if args.constraint_category and norm_stats is not None:
    ...
    # Create models — one per ensemble member, all sharing the architecture
    # detected from checkpoint[0].
    def _make_model():
        return create_maml_gcn_model(
            node_features=node_features,
    ...
    models = []
    for i, p in enumerate(resolved_model_paths):
        ck = checkpoint if i == 0 else torch.load(p, weights_only=False, map_location=device)
        m = _make_model()
    ...
    print(f"Loaded {K} ensemble member(s).")
    ...
    for cell_name in cell_list:
        result = run_cell_validation(args, cell_name, topology_cache, norm_stats, models, device)
    ...
    if args.save_results and all_results:
        ...
        for result in all_results:
            ...
            # Post-hoc per-cell conformal calibration suffix
            conf_suffix = f"_confA{args.conformal_alpha:g}" if args.conformal_alpha is not None else ""
            # Ensemble-size suffix (K>=2): distinguishes ensemble-mean outputs
            # from single-model outputs of the same config.
            ens_suffix = f"_K{K}" if K > 1 else ""
            # Conformal mode suffix only when ensemble is in use AND scalar
            # fallback was explicitly requested (otherwise default ensemble
            # mode is implied by the K>=2 setup).
            cmode_suffix = ""
            if args.conformal_alpha is not None and K > 1 and args.conformal_mode == 'scalar':
                cmode_suffix = "_confSCALAR"
            ...
            if args.model_type == 'baseline':
                base_name = f"{args.output_prefix}_{args.experiment}_{cell_name}_{file_data_type}_{args.graph_mode}_{args.mode}_{args.model_type}_iter{args.num_iterations}{arch_suffix}{filter_suffix}{voltage_suffix}{relpin_suffix}{adapt_suffix}{asym_suffix}{pin_suffix}{safe_suffix}{conf_suffix}{ens_suffix}{cmode_suffix}"
            else:
                base_name = f"{args.output_prefix}_{args.experiment}_{cell_name}_{file_data_type}_{args.graph_mode}_{args.mode}_{args.model_type}_innerdiv{args.innerdiv}_meta{args.tasks_per_meta_batch}_iter{args.num_iterations}_inner{args.inner_steps}{arch_suffix}{filter_suffix}{voltage_suffix}{relpin_suffix}{adapt_suffix}{asym_suffix}{pin_suffix}{safe_suffix}{conf_suffix}{ens_suffix}{cmode_suffix}"
