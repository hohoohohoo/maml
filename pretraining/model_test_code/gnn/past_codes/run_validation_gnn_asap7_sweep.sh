#!/bin/bash
# GNN Validation Sweep with Unified Dataset
# Runs validation across process-corner combinations and architectures
...
if [ $# -lt 1 ]; then
    ...
    echo ""
    echo "Example:"
    echo "  $0 json_configs/validation_gnn_sweep_config.json"
    echo "  $0 json_configs/validation_gnn_sweep_config.json --dry-run"
    echo ""
    echo "Note: Requires unified test dataset to be pre-generated."
    echo "      Run split_gnn_dataset.py --process <P> --corner <C> first."
    exit 1
fi
...
echo "=========================================="
echo "GNN Validation - Unified Dataset Sweep"
echo "=========================================="
...
PROCESSES=$(python3 -c "import json; print(' '.join(json.load(open('$CONFIG_FILE'))['sweep_params']['process']))")
...
for i, (proc, corner, conv_dim, conv_layers, fc_dim, fc_layers) in enumerate(combos, 1):
    print(f'  {i}. {proc}_{corner} - conv{conv_dim}x{conv_layers}_fc{fc_dim}x{fc_layers}')
"
    echo ""
...
for proc, corner, conv_dim, conv_layers, fc_dim, fc_layers in combos:
    print(f'{proc} {corner} {conv_dim} {conv_layers} {fc_dim} {fc_layers}')
" | while read PROC CORNER CONV_DIM CONV_LAYERS FC_DIM FC_LAYERS; do

    echo ""
    echo "=========================================="
    echo "Running: ${PROC}_${CORNER} - conv${CONV_DIM}x${CONV_LAYERS}_fc${FC_DIM}x${FC_LAYERS}"
    echo "=========================================="

    # Build command
    CMD="python -u ASAP7_GCN_voltage_validation.py"
    CMD="$CMD --process $PROC --corner $CORNER"
