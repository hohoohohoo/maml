#!/usr/bin/env python
"""
TSMC GAT Topology Validation Script

This script validates GAT models for TSMC topology experiments:
...
Usage:
  python TSMC_GAT_topology_validation.py --experiment intra_topology --model_type baseline --gpu 0
  python TSMC_GAT_topology_validation.py --experiment topology_agnostic --model_type maml --gpu 1
"""
...
def run_cell_validation(args, cell_name, topology_cache, norm_stats, model, device):
    ...
    # Build test directory suffix based on topology options
...
def main():
    parser = argparse.ArgumentParser(
        description='TSMC GAT Topology Validation',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python TSMC_GAT_topology_validation.py --experiment intra_topology --model_type baseline --gpu 0
  python TSMC_GAT_topology_validation.py --experiment topology_agnostic --model_type maml --gpu 1
  python TSMC_GAT_topology_validation.py --experiment intra_topology --model_type maml --mode interpolation
        """
    )
    ...
    parser.add_argument('--num_conv_layers', type=int, default=2,
                        help='Number of GAT convolutional layers (default: 2)')
    parser.add_argument('--fc_hidden_dim', type=int, default=64,
                        help='FC layer hidden dimension (default: 64)')
    ...
    parser.add_argument('--voltage_mode', type=str, default='all_nodes',
                        ...
                        help='Voltage mode: all_nodes (voltage on all nodes) or vdd_only (voltage only on VDD, 0 elsewhere) (default: all_nodes)')
    ...
    parser.add_argument('--inputport', action='store_true',
                        help='Use inputport topology (adds _inputport suffix to paths)')
    ...
    parser.add_argument('--output_prefix', type=str, default='TSMC_GAT',
    ...
    print(f"\n{'='*80}")
    print(f"TSMC GAT Topology Validation")
    print(f"{'='*80}")
    ...
    print(f"Inputport: {args.inputport}")
    print(f"Related pin only: {args.related_pin_only}")
    print(f"Cells: {len(cell_list)} cells")
    print(f"{'='*80}")

    # Load train data to get norm_stats and cache_path
    # Construct suffixes for paths
    ...
    topology_suffix = "_inputport" if args.inputport else ""
    slew_suffix = "_relpin" if args.related_pin_only else ""
    train_path = os.path.join(args.dataset_dir, f"train_{args.data_type}_{args.graph_mode}{topology_suffix}{slew_suffix}{voltage_suffix}{norm_suffix}.pth")
    ...
    # Load model
    print(f"\nLoading {args.model_type.upper()} GAT model...")

    if args.model_path is None:
        # Add pooling suffix only if not 'mean' (default)
        pool_suffix = f"_pool{args.pooling}" if args.pooling != 'mean' else ""
        arch_suffix = f"_conv{args.conv_hidden_dim}x{args.num_conv_layers}_fc{args.fc_hidden_dim}x{args.num_fc_layers}_heads{args.heads}{pool_suffix}"
        # Add gatectrl suffix if using gate control cache
        cache_suffix = "_gatectrl" if args.cache_path and "gatectrl" in args.cache_path else ""
        # Add inputport suffix to checkpoint directory
        checkpoint_topology_suffix = "_inputport" if args.inputport else ""
        checkpoint_slew_suffix = "_relpin" if args.related_pin_only else ""

        if args.model_type == 'baseline':
            model_dir = f"../../../pretrained_models/gat_baseline_checkpoints{voltage_suffix}{norm_suffix}{cache_suffix}{checkpoint_topology_suffix}{checkpoint_slew_suffix}"
            model_filename = f"gat_baseline_tsmc_process_{args.data_type}_{args.graph_mode}_iter{args.num_iterations}{arch_suffix}.pth"
        else:
            model_dir = f"../../../pretrained_models/gat_maml_tsmc_process_checkpoints{voltage_suffix}{norm_suffix}{cache_suffix}{checkpoint_topology_suffix}{checkpoint_slew_suffix}"
            model_filename = f"gat_maml_tsmc_process_{args.data_type}_{args.graph_mode}_innerdiv{args.innerdiv}_meta{args.tasks_per_meta_batch}_iter{args.num_iterations}_inner{args.inner_steps}{arch_suffix}.pth"
    ...
    # GAT uses 'convs.0.lin.weight' with shape [out_features, in_features]
    ...
    print(f"Architecture: conv={conv_hidden_dim}x{num_conv_layers}, fc={fc_hidden_dim}x{num_fc_layers}, heads={heads}, pooling={pooling_mode}")
    ...
    if args.save_results and all_results:
        ...
        topo_suffix = "_inputport" if args.inputport else ""
        ...
        for result in all_results:
            ...
            if args.model_type == 'baseline':
                base_name = f"{args.output_prefix}_{args.experiment}_{cell_name}_{args.data_type}_{args.graph_mode}{topo_suffix}{relpin_suffix}_{args.mode}_{args.model_type}_iter{args.num_iterations}{arch_suffix}{filter_suffix}{adapt_suffix}"
            else:
                base_name = f"{args.output_prefix}_{args.experiment}_{cell_name}_{args.data_type}_{args.graph_mode}{topo_suffix}{relpin_suffix}_{args.mode}_{args.model_type}_innerdiv{args.innerdiv}_meta{args.tasks_per_meta_batch}_iter{args.num_iterations}_inner{args.inner_steps}{arch_suffix}{filter_suffix}{adapt_suffix}"
