...
class CellTestDataset:
    ...
    Adapted for ASAP7 Process dataset format.
    """
...
def run_cell_validation(args, cell_name, topology_cache, norm_stats, model, device):
    ...
    cell_filename = f"cell_{cell_name}_ASAP7_75t_L_graph_data_{args.graph_mode}.pth"
    cell_path = os.path.join(
        args.dataset_dir,
        f"test_by_cell_{args.graph_mode}",
        cell_filename
    )
            ...
            if y_std > 0:
                ...
                if abs(max_val - min_val) > 0:
                    ...
                    if i % 100 == 0 and i > 0:
                        ...
                        print(f"     Current avg MAE: {1000*sum(total_mae)/len(total_mae):.3f}ps - Tasks: {len(total_mae)}", flush=True)
    ...
    print(f"   {cell_name}: NRMSE={results['nrmse_total']:.2f}%, MAPE={results['mape_total']:.2f}%, "
          f"MAE={results['mae_total']*1e9:.4f}ns ({len(total_nrmse)} tasks)")
  ...
  python ASAP7_GCN_topology_validation.py --experiment intra_topology --model_type maml --mode interpolation
    ...
    if args.model_path is None:
        ...
        if args.model_type == 'baseline':
            model_dir = "../../../pretrained_models/gnn_baseline_asap7_process_checkpoints"
            model_filename = f"gnn_baseline_asap7_process_{args.data_type}_{args.graph_mode}_iter{args.num_iterations}{arch_suffix}.pth"
        else:
            model_dir = "../../../pretrained_models/gnn_maml_asap7_process_final"
    ...
    if all_results:
        ...
        print("-" * 70)
        print(f"{'AVERAGE':<25} {total_tasks:<8} {avg_nrmse:<10.2f} {avg_mape:<10.2f} {avg_mae*1e9:<12.4f}")
        ...
        print(f"  MAE   - Left: {sum(r['mae_left'] for r in all_results)/len(all_results)*1e9:.4f}ns, "
              f"Right: {sum(r['mae_right'] for r in all_results)/len(all_results)*1e9:.4f}ns, "
              f"Inter: {sum(r['mae_inter'] for r in all_results)/len(all_results)*1e9:.4f}ns")
