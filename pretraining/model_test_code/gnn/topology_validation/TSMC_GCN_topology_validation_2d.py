#!/usr/bin/env python
"""
TSMC GCN Topology Validation Script — 2-D V×T variant (option α: flat random K)

Mirrors TSMC_GCN_topology_validation.py but reads 2-D V×T test datasets in
...
Usage:
  python TSMC_GCN_topology_validation_2d.py --experiment intra_topology --model_type maml --gpu 1
  python TSMC_GCN_topology_validation_2d.py --experiment topology_agnostic --model_type maml --gpu 1
"""
...
def _build_argparser():
    """CLI parser for TSMC GCN Topology Validation (2-D V×T)."""
...
Examples:
  ...
  python TSMC_GCN_topology_validation.py --experiment intra_topology --model_type maml --mode interpolation
    ...
    parser.add_argument('--dataset_dir', type=str,
                        default='/home/tkdgn2907/Deepsets_test/MAML/Projects/dataset_all/gnn_dataset_2d',
    ...
    parser.add_argument('--output_prefix', type=str, default='TSMC_GCN_2d',
...
def _load_train_data_and_norm_stats(args, suffixes):
    """
    Locate the 2-D train dataset, load with mmap.

    Returns (norm_stats, cache_path_from_data) on success; (None, None) if the
    train file does not exist.
    """
    ...
    train_path = os.path.join(
        args.dataset_dir,
        f"train_{args.data_type}_{args.graph_mode}{top}{volt}{norm}{temp}{slew}_2d.pth",
    )
...
def _resolve_model_path(args, suffixes, checkpoint_suffix):
    ...
    if args.model_type == 'baseline':
        model_dir = (f"../../../pretrained_models/gnn_baseline_tsmc_process_2d_checkpoints"
                     f"{checkpoint_suffix}{volt}{norm}{temp}{slew}")
        model_filename = (f"gnn_baseline_tsmc_process_2d_{args.data_type}_{args.graph_mode}"
                          f"_iter{args.num_iterations}{arch_suffix}.pth")
    else:
        model_dir = (f"../../../pretrained_models/gnn_maml_tsmc_process_2d_checkpoints"
                     f"{checkpoint_suffix}{volt}{norm}{temp}{slew}")
        model_filename = (f"gnn_maml_tsmc_process_2d_{args.data_type}_{args.graph_mode}"
...
def _load_gnn_model_from_checkpoint(model_path, args, device):
    ...
    # node_features is derived from checkpoint weight shape (2-D TSMC dataset may be 12-D)
...
def _resolve_output_dir(args):
    """Map --output_dir arg to actual directory name (final/default/custom)."""
...
def _save_results_npy(args, all_results, arch_kwargs):
    ...
    for result in all_results:
        ...
        np.save(f"{output_dir}/{base_name}_pred.npy", result['predictions'])
        np.save(f"{output_dir}/{base_name}_act.npy", result['actuals'])
