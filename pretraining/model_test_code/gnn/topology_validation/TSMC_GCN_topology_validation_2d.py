...
Examples:
  ...
  python TSMC_GCN_topology_validation.py --experiment intra_topology --model_type maml --mode interpolation
    ...
    temp_suffix = "_temp_all" if args.temp_mode == "temp_all" else ""
    slew_suffix = "_relpin" if args.related_pin_only else ""
    train_path = os.path.join(args.dataset_dir, f"train_{args.data_type}_{args.graph_mode}{topology_suffix}{voltage_suffix}{norm_suffix}{temp_suffix}{slew_suffix}_2d.pth")
    ...
    if args.model_path is None:
        ...
        if args.model_type == 'baseline':
            model_dir = f"../../../pretrained_models/gnn_baseline_tsmc_process_2d_checkpoints{checkpoint_suffix}{voltage_suffix}{norm_suffix}{temp_suffix}{slew_suffix}"
            model_filename = f"gnn_baseline_tsmc_process_2d_{args.data_type}_{args.graph_mode}_iter{args.num_iterations}{arch_suffix}.pth"
        else:
            model_dir = f"../../../pretrained_models/gnn_maml_tsmc_process_2d_checkpoints{checkpoint_suffix}{voltage_suffix}{norm_suffix}{temp_suffix}{slew_suffix}"
            model_filename = f"gnn_maml_tsmc_process_2d_{args.data_type}_{args.graph_mode}_innerdiv{args.innerdiv}_meta{args.tasks_per_meta_batch}_iter{args.num_iterations}_inner{args.inner_steps}{arch_suffix}.pth"
