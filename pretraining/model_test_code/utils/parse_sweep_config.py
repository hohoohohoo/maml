#!/usr/bin/env python3
"""
Parse sweep configuration JSON and generate experiment combinations
"""
    ...
    if 'sweep_params' in config:
        ...
        for i, combo in enumerate(itertools.product(*param_values), 1):
            ...
            for param_name, param_value in zip(param_names, combo):
                # Handle list parameters (e.g., cells)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: parse_sweep_config.py <config.json>", file=sys.stderr)
        sys.exit(1)
