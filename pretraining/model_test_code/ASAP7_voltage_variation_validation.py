...
"""
ASAP7 Voltage Variation Validation with MLP/MAML

This script validates MLP or MAML model performance on ASAP7 voltage variation test datasets.
Supports multiple cell types (LVT/RVT/SLVT/SRAM) and corner conditions (SS/FF/TT).
...
def main():
    ...
    parser = argparse.ArgumentParser(description='ASAP7 Voltage Variation Validation with MLP/MAML')
    ...
    parser.add_argument('--meta', type=int, default=16,
                        help='MAML tasks per meta batch (default: 16)')
    ...
    if not os.path.exists(model_path):
        ...
        print("Please ensure the model has been trained with the pretraining script")
        return
    ...
    for i, randomtask in enumerate(test_indices):
        if randomtask not in discontinuous_task_ids:
            ...
            try:
                ...
                # Define regions based on left_bound and right_bound
    ...
    # Print final results
    print(f"\n{'='*80}")
    print(f"Final Results on {len(total_nrmse)} valid tasks")
    print(f"{'='*80}")
    ...
    if len(total_nrmse) > 0:
        ...
        if args.save_results:
            ...
            if model_framework == 'mlp':
                pred_filename = f"data_result_npy_directory/ASAP7_voltage_variation_{cell_type}_{corner}_{data_type}_{model_type}_pred.npy"
                act_filename = f"data_result_npy_directory/ASAP7_voltage_variation_{cell_type}_{corner}_{data_type}_{model_type}_act.npy"
            else:  # maml
                pred_filename = f"data_result_npy_directory/ASAP7_voltage_variation_{cell_type}_{corner}_{data_type}_maml_innerdiv{args.innerdiv}_meta{args.meta}_layer{args.layer_length}_inner{args.inner_step}_pred.npy"
                act_filename = f"data_result_npy_directory/ASAP7_voltage_variation_{cell_type}_{corner}_{data_type}_maml_innerdiv{args.innerdiv}_meta{args.meta}_layer{args.layer_length}_inner{args.inner_step}_act.npy"
