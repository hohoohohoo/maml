...
parser.add_argument('--peripheral_mode', type=str, default='grad_adam',
...
BASELINE_MODEL_PATHS = {
    ...
    #'cell': '/home/tkdgn2907/Deepsets_test/MAML/Projects/CAD_TEST/trained_models_lib/checkpoints/cell_lib_innerdiv100_meta32_local_norm_3hidden_(40)_10000_inner1.pth',
    ...
    #'transition' : '/home/tkdgn2907/Deepsets_test/MAML/Projects/CAD_TEST/trained_models_lib/transition_lib_innerdiv100_meta32_local_norm_3hidden_(40)_10000_inner1.pth'
}
...
def run_validation_with_threshold(data_type, all_data, valid_tables_by_group, maml_model, threshold):
    ...
    for group_key, conditions in VOLTAGE_SWEEP_GROUPS.items():
        ...
        group_name = f"{corner.upper()}_{temp.upper()}"

        if group_name not in valid_tables_by_group:
            continue

        valid_tables = valid_tables_by_group[group_name]
        if not valid_tables:
            continue
        ...
        for table_key in sample_tables:
            ...
            adapted_model, y_mean_center, y_std_center, grad_center, move_center, _, _, _, ref_idx = adapt_with_center_point(
                ...
                num_steps=args.center_steps, lr=args.center_lr
            )
