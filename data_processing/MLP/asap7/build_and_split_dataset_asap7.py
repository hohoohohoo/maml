...
"""
Unified dataset builder for separate cell types supporting both cell and transition delay types.

This script builds datasets with train/test splits by cell type, supporting both:
- Cell delay: Uses libdata_extract_MAML_cell module
- Transition delay: Uses libdata_extract_MAML_transition module

The delay type is specified via --delay-type argument.
"""
...
def filter_pin_data_by_cell_type(pin_data, target_cell_types):
    ...
    # If target_cell_types contains "NONE" or empty/whitespace strings, put all data in train set
    if "NONE" in target_cell_types or len(target_cell_types) == 0 or all(t.strip() == "" for t in target_cell_types):
...
def parse_ex2_folder(folder_path):
    ...
    # Pattern 3: VDD_<voltage>_<a>_<b>_<process>_<temp>_<corner>
...
def build_split_dataset_by_cell_type(filenames, abc_param_lists, target_cell_types, folder_params,
                                   ...
                                   parse_liberty_fn, flatten_pin_fn):
    """
    ...
    Args:
        parse_liberty_fn: Function to parse liberty pin blocks (from delay-type specific module)
        flatten_pin_fn: Function to flatten pin data (from delay-type specific module)
    """
    ...
    for i, filename in enumerate(filenames):
        ...
        if test_pins:
            test_flattened, test_cap = flatten_pin_fn(test_pins)
        ...
        if train_pins:
            train_flattened, train_cap = flatten_pin_fn(train_pins)
...
def create_separate_test_sets(folder_paths, abc_param_lists, target_cell_types, output_dir,
                             delay_type, parse_liberty_fn, flatten_pin_fn):
    ...
    for target_cell_type in target_cell_types:
        ...
        if all_test_inputs and all_test_outputs:
            ...
            try:
                ...
                test_input_path = f"{output_dir}/{delay_type}_{target_cell_type}_test_input.pth"
                test_output_path = f"{output_dir}/{delay_type}_{target_cell_type}_test_output.pth"
...
def create_shared_train_set(folder_paths, abc_param_lists, exclude_cell_types, output_dir,
                           delay_type, topology_type, parse_liberty_fn, flatten_pin_fn):
    """
    Create shared train set excluding all test cell types
    각 폴더마다 61개 lib file을 개별 처리하고 스택해서 x,61,9 구조 생성

    Args:
        topology_type: 'intra' or 'agnostic' to determine train file naming
    """
    ...
    if all_train_inputs and all_train_outputs:
        ...
        # Determine train file naming based on topology type
        if topology_type == 'intra':
            train_suffix = 'intratopology'
        else:  # agnostic
            train_suffix = 'topology_agnostic'

        # Save shared train data
        train_input_path = f"{output_dir}/{delay_type}_{train_suffix}_train_input.pth"
        train_output_path = f"{output_dir}/{delay_type}_{train_suffix}_train_output.pth"
...
def main():
    ...
    parser.add_argument('--topology-type', type=str, default='agnostic', choices=['intra', 'agnostic'],
                       help='Topology type for train file naming: intra or agnostic (default: agnostic)')
    parser.add_argument('--train-only', action='store_true', help='Only create shared train dataset')

    args = parser.parse_args()

    # Import appropriate modules based on delay type
    if args.delay_type == 'cell':
        from utils.libdata_extract_MAML_cell import parse_liberty_pin_blocks, flatten_pin_data
        print("📦 Using libdata_extract_MAML_cell module for cell delay")
    else:  # transition
        from utils.libdata_extract_MAML_transition import parse_liberty_pin_blocks, flatten_pin_data
        print("📦 Using libdata_extract_MAML_transition module for transition delay")
    ...
    print(f"⏱️  Delay type: {args.delay_type}")
    print(f"🏗️  Topology type: {args.topology_type}")
    ...
    for data_dir in args.data_dirs:
        ...
        for p in Path(data_dir).iterdir():
            if p.is_dir() and ( p.name.startswith('simple_') or p.name.startswith('invbuf_')):
    ...
    if args.train_only:
        ...
        create_shared_train_set(folder_paths, abc_param_lists, args.test_cell_types, args.output_dir,
                              args.delay_type, args.topology_type, parse_liberty_pin_blocks, flatten_pin_data)
    else:
        ...
        create_separate_test_sets(folder_paths, abc_param_lists, args.test_cell_types, args.output_dir,
                                 args.delay_type, parse_liberty_pin_blocks, flatten_pin_data)

        # Create shared train set (excluding all test cell types)
        # Note: Commented out in original scripts
        # create_shared_train_set(folder_paths, abc_param_lists, args.test_cell_types, args.output_dir,
        #                       args.delay_type, args.topology_type, parse_liberty_pin_blocks, flatten_pin_data)
