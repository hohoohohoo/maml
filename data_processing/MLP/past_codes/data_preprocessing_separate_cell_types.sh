...
#TEST_CELL_TYPES=(" ")  # Each will create separate test files
TEST_CELL_TYPES=("AND2x6" "NAND3x2" "NOR2xp67" "OR2x6")  # Each will create separate test filesecho "🚀 Creating separate test sets for each cell type..."
...
if [[ "$1" == "--train-only" ]]; then
    ...
    for CELL_TYPE in "${TEST_CELL_TYPES[@]}"; do
        ...
        python3 build_and_split_dataset_separate_cell_types.py \
