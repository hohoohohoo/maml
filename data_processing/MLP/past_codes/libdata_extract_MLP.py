import re
import pandas as pd
from transform_sample2 import transform_all_samples
    ...
    for line in lines:
        ...
        if line.startswith("cell"):
            ...
            if match:
                ...
                # "AND2x2" 처럼 앞쪽의 논리 gate 타입만 추출하고 싶다면:
                logic_type = re.match(r'[A-Z]+', full_cell_name)
                #print(full_cell_name)
                current_cell_name = logic_type.group(0) if logic_type else full_cell_name
        ...
        # timing 내부 처리
            ...
            #    current_timing["index_1"] = list(map(float, re.findall(r'[\d.]+', line)))
            #elif "index_2" in line and inside_delaytype:
            #    current_timing["index_2"] = list(map(float, re.findall(r'[\d.]+', line)))
...
# 파일 읽기
#with open("4cells_2_0_070 .lib", "r") as f:
#        lines = f.readlines()
# 파싱 실행
    ...
    for pin in pin_data:
       ...
       # related_power_pin = pin.get("related_power_pin","")
        ...
        for t in pin.get("timings", []):
            for dtype in ["cell_rise", "cell_fall", "rise_transition", "fall_transition"]:
            #for dtype in ["cell_rise"]:
                if dtype in t:
                    rows.append({
                        "Process":Process,
                        "Temperature":Temperature,
...
# 저장
#df.to_csv("pin_data_output.csv", index=False)
