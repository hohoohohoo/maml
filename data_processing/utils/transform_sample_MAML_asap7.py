...
def transform_sample(sample, cap, cell_types, delay_types, size_types, port_nums, lib_prefix="", abc_params=None):
    ...
    # Check if lib_prefix starts with invbuf for additional_dim logic
    additional_dim = 0
    if (lib_prefix.startswith("invbuf") or lib_prefix.startswith("ex2") or lib_prefix.startswith("simple")) and cell_index != -1:
    ...
    a_n_param = abc_params.get('a_n', abc_params.get('a', 0.0)) if abc_params else 0.0  # nmos version
    a_p_param = abc_params.get('a_p', abc_params.get('a', 0.0)) if abc_params else 0.0  # pmos version
    ...
    if len(values_2d) > 0 and len(values_2d[0]) > 0:
        ...
        for row_idx in range(rows):
            for col_idx in range(cols):
                ...
                input_array = [
                    a_param,        # Feature 6 (new)
                    b_param,        # Feature 7 (new)
                    c_param,        # Feature 8 (new)
                    temperature,     # Feature 9 (new)
                    voltage,        # Feature 1
                    additional_dim, # Feature 2  
                    delay_indicator,# Feature 3
                    index_1_val,    # Feature 4
                    index_2_val    # Feature 5
