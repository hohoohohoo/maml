...
# Step 1.5: Weighted adjacency matrix 옵션
echo ""
echo "Weighted Adjacency Matrix Option:"
...
if [ "$weighted_choice" == "2" ]; then
    ...
    echo "      [1] No (default) - 7D node features"
    echo "      [2] Yes - 8D node features (adds parasitic cap sum per node)"
    read -p "   Enter choice (1/2) [default: 1]: " cap_choice
    if [ "$cap_choice" == "2" ]; then
        INCLUDE_CAP_FLAG="--include_parasitic_cap"
        echo "   Include parasitic cap: YES (8D features)"
    else
        echo "   Include parasitic cap: NO (7D features)"
    fi
else
    echo "   Selected: Binary adjacency (0/1)"
fi

# Update cache file name with weighted suffix
CACHE_FILE="$CACHE_DIR/${CACHE_PREFIX}_${SPI_BASENAME}${WEIGHTED_SUFFIX}.pth"
