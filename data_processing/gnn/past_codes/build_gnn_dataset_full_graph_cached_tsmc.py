...
def dataextract_gnn_cached_tsmc(lib_file_path, topology_cache, lib_prefix="", data_type="cell", cache_type=None,
    ...
    Args:
        ...
        include_parasitic_cap: If True, add parasitic capacitance as additional feature (8D instead of 7D)
        ...
        for row_idx in range(effective_rows):
            for col_idx in range(effective_cols):
                ...
                if include_parasitic_cap:
                    ...
                    # Get all_nodes from graph_sample for node name lookup
                    ...
                    for node_idx, node_name in enumerate(all_nodes):
                        ...
                        elif node_name.startswith('XM'):
                            # For transistor nodes, try terminal formats
                            mos_name = node_name.replace('X', '')
                            total_cap = 0.0
                    ...
                    node_features = torch.cat([node_features, cap_features], dim=1)
...
def build_all_gnn_data_cached_tsmc(
    ...
    num_features = 8 if include_parasitic_cap else 7
    print("=" * 80)
    ...
    print(f"Node features: {num_features}D ({'7 base + 1 cap' if include_parasitic_cap else '7 base'})")
    print("=" * 80)
...
"""
    ...
    parser.add_argument("--include_parasitic_cap", action="store_true",
                       help="Include parasitic capacitance as additional node feature (8D instead of 7D). "
