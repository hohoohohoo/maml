#!/usr/bin/env python
"""
Compare and visualize GNN validation sweep results for TSMC dataset.
...
Location: Projects/result_management/compare_gcn_validation_results_tsmc.py

Usage:
    python compare_gcn_validation_results_tsmc.py --results_dir 10000samples --mode interpolation
    python compare_gcn_validation_results_tsmc.py --results_dir 10000samples --mode interpolation --filter "FF_0"
    python compare_gcn_validation_results_tsmc.py --results_dir 10000samples --mode interpolation --corner FF --temperature 0
"""
...
# MLP MAML baseline directory
MLP_MAML_BASELINE_DIR = '/home/tkdgn2907/Deepsets_test/MAML/Projects/pretraining/model_test_code/data_result_npy_directory'


def load_mlp_maml_baseline(corner, temperature, data_type='cell', group_size=61):
    """
    Load MLP MAML baseline results for comparison.
    ...
    # Find MAML files matching the corner and temperature
    ...
    if not pred_files:
        print(f"  Warning: No MLP MAML baseline found for {corner}_{temperature}")
        return None
...
def parse_filename(filename):
    """
    Parse TSMC validation result filename to extract configuration.

    Example filenames:
    TSMC_GCN_unified_TSMC_FF_0_cell_full_graph_interpolation_baseline_iter100000_conv128x3_fc128x2_pred.npy
    TSMC_GCN_unified_TSMC_TT_25_cell_stage_aware_interpolation_maml_innerdiv10_meta16_iter30000_inner1_conv64x3_fc40x2_pred.npy
    """
    ...
    # Extract TSMC corner and temperature
    # Pattern: TSMC_{corner}_{temperature}
    tsmc_match = re.search(r'TSMC_([A-Z]+)_(\d+)', name)
    if tsmc_match:
        config['process'] = 'TSMC'  # For compatibility
        config['corner'] = tsmc_match.group(1)
        config['temperature'] = int(tsmc_match.group(2))
    else:
        # Fallback: try to extract corner only (for backward compatibility)
        for corner in ['TT', 'FF', 'SS']:
            if f'_{corner}_' in name:
                config['corner'] = corner
                break

    # Extract data_type (cell or transition)
    if '_cell_' in name:
        config['data_type'] = 'cell'
    elif '_transition_' in name:
        config['data_type'] = 'transition'
    else:
    ...
    # Create architecture label with model_type, graph_mode, corner, temperature
    ...
    if 'conv_hidden_dim' in config and 'fc_hidden_dim' in config:
        ...
        config['arch_label'] = f"{model_short}_{graph_short}_{corner}{temperature}_{arch_params}"
    ...
    config['full_label'] = f"{model_type}_{graph_mode}_{corner}_{temperature}_conv{config.get('conv_hidden_dim', '?')}x{config.get('num_conv_layers', '?')}_fc{config.get('fc_hidden_dim', '?')}x{config.get('num_fc_layers', '?')}"
...
def plot_metrics_comparison(results, output_dir, metric_name='nrmse', mlp_maml_baseline=None, crop_to_baseline=False):
    ...
    for plot_type in ['full', 'top5', 'top5_FG', 'top5_SA']:
        ...
        if metric_name == 'mae':
            ...
            ax.set_title(f'MAE (x1000) Comparison Across Architectures (TSMC){title_suffix}', fontsize=14)
        else:
            ax.set_ylabel(metric_name.upper(), fontsize=12)
            ax.set_title(f'{metric_name.upper()} Comparison Across Architectures (TSMC){title_suffix}', fontsize=14)
        ax.set_xticks(range(len(labels)))
        ax.set_xticklabels(labels, rotation=45, ha='right', fontsize=8 if plot_type == 'full' else 10)
        ...
        if mlp_maml_baseline is not None and metric_name in mlp_maml_baseline:
            ...
            if crop_to_baseline:
                # Get current y-axis minimum from data
...
def plot_all_metrics_heatmap(results, output_dir):
    ...
    metrics_to_show = ['nrmse', 'mape', 'smape', 'mae', 'r2']
    ...
    _fig, ax = plt.subplots(figsize=(12, max(6, len(results) * 0.4)))

    _im = ax.imshow(data_normalized, cmap='RdYlGn_r', aspect='auto')

    ax.set_xticks(range(len(metrics_to_show)))
    # MAE는 x1000 표시
    xlabels = [m.upper() if m != 'mae' else 'MAE(x1000)' for m in metrics_to_show]
    ...
    for i in range(len(results)):
        for j in range(len(metrics_to_show)):
            ...
            if metrics_to_show[j] == 'r2':
                ...
                text = f'{val*1000:.4f}'  # MAE x1000, 4 decimal places
            else:
                text = f'{val:.4f}'  # 4 decimal places for all
            ax.text(j, i, text, ha='center', va='center', fontsize=7,
                   color='white' if data_normalized[i, j] > 0.5 else 'black')

    ax.set_title('Metrics Comparison Heatmap - TSMC\n(Green=Better, Red=Worse)', fontsize=12)
...
def plot_architecture_comparison_by_params(results, output_dir):
    ...
    mape_vals = [r['metrics']['mape'] for r in results]
    ...
    # Plot 3: MAPE vs conv_hidden_dim
    ax = axes[1, 0]
    for fc in sorted(set(fc_dims)):
        mask = [f == fc for f in fc_dims]
        x = [c for c, m in zip(conv_dims, mask) if m]
        y = [n for n, m in zip(mape_vals, mask) if m]
        if x:
            ax.scatter(x, y, label=f'fc_dim={fc}', s=50)
    ax.set_xlabel('Conv Hidden Dim')
    ax.set_ylabel('MAPE (%)')
    ax.set_title('MAPE vs Conv Hidden Dim')
    ax.legend()
    ax.grid(True, alpha=0.3)

    # Plot 4: MAPE vs fc_hidden_dim
    ax = axes[1, 1]
    for conv in sorted(set(conv_dims)):
        mask = [c == conv for c in conv_dims]
        x = [f for f, m in zip(fc_dims, mask) if m]
    ...
    ax.set_xlabel('FC Hidden Dim')
    ax.set_ylabel('MAPE (%)')
    ax.set_title('MAPE vs FC Hidden Dim')
    ax.legend()
    ax.grid(True, alpha=0.3)

    plt.suptitle('Metrics vs Architecture Parameters (TSMC)', fontsize=14)
    plt.tight_layout()
...
def plot_parameter_trend_analysis(results, output_dir, mlp_maml_baseline=None, crop_to_baseline=False):
    ...
    1. Fix conv_hidden_dim, vary fc_hidden_dim
    2. Fix fc_hidden_dim, vary conv_hidden_dim

    For each metric (NRMSE, SMAPE, MAE), shows line plots with trends.
    ...
    # ========================================
    # Create versions: FG/SA x layer_config x full/filtered (excluding conv_dim=256)
    graph_mode_configs = [
        ('full_graph', 'FG'),
        ('stage_aware', 'SA')
    ]
    ...
    # Get unique layer configurations
        ...
        for n_conv, n_fc in layer_configs:
            ...
            for _version, conv_dims_to_plot, conv_suffix in conv_filter_configs:
                ...
                for show_annotations, annot_suffix in annotation_configs:
                    ...
                    fig.suptitle(f'Trend Analysis: Fix conv_hidden_dim, Vary fc_hidden_dim{title_suffix} (TSMC)', fontsize=14, fontweight='bold')

                    for idx, metric in enumerate(metrics):
                        ax = axes[idx]

                        for i, conv_dim in enumerate(conv_dims_to_plot):
                            # Get results with this conv_dim
                            filtered = [r for r in layer_results if r['config'].get('conv_hidden_dim') == conv_dim]

                            if not filtered:
                                continue

                            # Group by fc_hidden_dim and get average metric
                            fc_metric_data = {}
                            for r in filtered:
                            ...
                            # Calculate mean for each fc_dim
                            ...
                            # Plot line with markers (no error bars)
                        ...
                        if metric in baseline_values:
                            ...
                            if crop_to_baseline:
                                # Fit to data range (use current y-axis min)
                                y_min, _ = ax.get_ylim()
        ...
        for n_conv, n_fc in layer_configs:
            ...
            for _version, conv_dims_to_include, conv_suffix in conv_filter_configs:
                ...
                for show_annotations, annot_suffix in annotation_configs:
                    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
                    title_suffix = f' [{mode_label}] (layers:{n_conv}conv,{n_fc}fc)'
                    fig.suptitle(f'Trend Analysis: Fix fc_hidden_dim, Vary conv_hidden_dim{title_suffix} (TSMC)', fontsize=14, fontweight='bold')

                    for idx, metric in enumerate(metrics):
                        ax = axes[idx]

                        for i, fc_dim in enumerate(fc_dims):
                            # Get results with this fc_dim (and filter conv_dims)
                            ...
                            # Group by conv_hidden_dim and get average metric
                            conv_metric_data = {}
                            for r in filtered:
    ...
    # ========================================
    # Plot 2.5: Compare layer configurations (2x2, 2x3, 3x2, 3x3)
    # ========================================
    # For each graph_mode and conv_filter, compare all layer configs
    layer_colors = {
        (2, 2): 'tab:blue',
        (2, 3): 'tab:orange',
        (3, 2): 'tab:green',
    ...
    for graph_mode, mode_label in graph_mode_configs:
        ...
        for version, conv_dims_to_plot, conv_suffix in conv_filter_configs:
            if not conv_dims_to_plot:
                continue

            # Plot 2.5a: Fix conv_dim, vary fc_dim - compare layer configs
            for show_annotations, annot_suffix in annotation_configs:
                fig, axes = plt.subplots(1, 3, figsize=(18, 5))
                title_suffix = f' [{mode_label}]'
                fig.suptitle(f'Layer Config Comparison: Fix conv_dim, Vary fc_dim{title_suffix} (TSMC)', fontsize=14, fontweight='bold')
                ...
                for idx, metric in enumerate(metrics):
                    ...
                    for (n_conv, n_fc), color in layer_colors.items():
                        ...
                        # Average across all conv_dims for each fc_dim
                        fc_metric_data = {}
            ...
            # Plot 2.5b: Fix fc_dim, vary conv_dim - compare layer configs
            for show_annotations, annot_suffix in annotation_configs:
                fig, axes = plt.subplots(1, 3, figsize=(18, 5))
                title_suffix = f' [{mode_label}]'
                fig.suptitle(f'Layer Config Comparison: Fix fc_dim, Vary conv_dim{title_suffix} (TSMC)', fontsize=14, fontweight='bold')
                ...
                for idx, metric in enumerate(metrics):
                    ...
                    for (n_conv, n_fc), color in layer_colors.items():
                        ...
                        # Average across all fc_dims for each conv_dim
                        conv_metric_data = {}
                        for r in layer_results:
                            conv_dim = r['config'].get('conv_hidden_dim')
                        ...
                        # Use indices for equal spacing
                        x_indices = [conv_dims_to_plot.index(conv) for conv in x_vals_raw]
            ...
            # Plot 2.5c: Bar chart - Best metric for each layer config
            for show_annotations, annot_suffix in annotation_configs:
                fig, axes = plt.subplots(1, 3, figsize=(18, 5))
                title_suffix = f' [{mode_label}]'
                fig.suptitle(f'Best Performance by Layer Configuration{title_suffix} (TSMC)', fontsize=14, fontweight='bold')
                ...
                for idx, metric in enumerate(metrics):
                    ...
                    ax.set_ylabel(metric_labels[metric], fontsize=11, fontweight='bold')
                    ax.set_title(f'Best {metric_labels[metric]} by Layer Config', fontsize=12)
                    ax.set_xticks(x_pos)
    ...
    # ========================================
    # Plot 3: Fix num_conv_layers, vary other params (heatmap style)
    # ========================================
    for num_conv in num_conv_layers_list:
        filtered = [r for r in results if r['config'].get('num_conv_layers') == num_conv]
        if len(filtered) < 2:
            continue

        fig, axes = plt.subplots(1, 3, figsize=(18, 5))
        fig.suptitle(f'Trend Analysis: num_conv_layers={num_conv} (TSMC)', fontsize=14, fontweight='bold')

        for idx, metric in enumerate(metrics):
            ax = axes[idx]

            # Group by (conv_dim, fc_dim)
            for i, conv_dim in enumerate(conv_dims):
                conv_filtered = [r for r in filtered if r['config'].get('conv_hidden_dim') == conv_dim]
                if not conv_filtered:
                    continue

                fc_metric_data = {}
                for r in conv_filtered:
                    fc_dim = r['config'].get('fc_hidden_dim')
                    if fc_dim not in fc_metric_data:
                        fc_metric_data[fc_dim] = []
                    val = r['metrics'][metric]
                ...
                color = colors[i % len(colors)]
                marker = markers[i % len(markers)]
                ax.plot(x_indices, y_vals, label=f'conv={conv_dim}',
                       color=color, marker=marker, markersize=8, linewidth=2)
        ...
        plt.tight_layout()
        output_path = os.path.join(output_dir, f'trend_num_conv_{num_conv}.png')
        plt.savefig(output_path, dpi=150, bbox_inches='tight')
        print(f"Saved: {output_path}")
        plt.close()

    # ========================================
    # Plot 4: Summary bar chart - Best config for each conv_dim
    # ========================================
    for show_annotations, annot_suffix in annotation_configs:
        fig, axes = plt.subplots(1, 3, figsize=(18, 5))
        fig.suptitle('Best Configuration by conv_hidden_dim (TSMC)', fontsize=14, fontweight='bold')
        ...
        for idx, metric in enumerate(metrics):
            ...
            for conv_dim in conv_dims:
                ...
                # Find best by this metric (lower is better for error metrics)
                ...
                best_configs.append({
                    ...
                    'best_val': val,
                    'arch_label': best['config'].get('arch_label', '')
                })
            ...
            if show_annotations:
                # Add value and best fc_dim labels
            ...
            ax.set_xlabel('conv_hidden_dim', fontsize=11, fontweight='bold')
            ax.set_ylabel(metric_labels[metric], fontsize=11, fontweight='bold')
            ax.set_title(f'Best {metric_labels[metric]} by conv_dim', fontsize=12)
    ...
    # ========================================
    # Plot 5: Summary bar chart - Best config for each fc_dim
    # ========================================
    for show_annotations, annot_suffix in annotation_configs:
        fig, axes = plt.subplots(1, 3, figsize=(18, 5))
        fig.suptitle('Best Configuration by fc_hidden_dim (TSMC)', fontsize=14, fontweight='bold')
        ...
        for idx, metric in enumerate(metrics):
            ...
            if show_annotations:
                # Add value and best conv_dim labels
                for i, (bar, label, val) in enumerate(zip(bars, labels, y_vals)):
                    height = bar.get_height()
                    ax.annotate(f'{val:.3f}\n({label})',
                               xy=(bar.get_x() + bar.get_width() / 2, height),
            ...
            ax.set_xlabel('fc_hidden_dim', fontsize=11, fontweight='bold')
            ax.set_ylabel(metric_labels[metric], fontsize=11, fontweight='bold')
            ax.set_title(f'Best {metric_labels[metric]} by fc_dim', fontsize=12)
            ax.set_xticks(range(len(x_vals)))
            ax.set_xticklabels(x_vals)
            ax.legend(loc='best', fontsize=9)
            ax.grid(True, alpha=0.3, axis='y')
...
def save_parameter_trend_summary(results, output_dir):
    """
    Save parameter trend analysis summary to text file.
    Shows results sorted by accuracy (best first) for each metric.
    """
    if not results:
        return
    ...
    lines = []
    lines.append("=" * 100)
    lines.append("PARAMETER TREND ANALYSIS SUMMARY (TSMC)")
    lines.append("=" * 100)
    lines.append("")
    ...
    # ========================================
    # Section 1: Overall Ranking (sorted by each metric)
    # ========================================
    ...
    # ========================================
    # Section 2: Best configuration for each conv_hidden_dim
    # ========================================
    lines.append("=" * 100)
    lines.append("BEST CONFIGURATION FOR EACH conv_hidden_dim")
    lines.append("=" * 100)
    lines.append("")
    ...
    for conv_dim in conv_dims:
        ...
        lines.append(f"--- conv_hidden_dim = {conv_dim} ({len(filtered)} configurations) ---")
        lines.append("")

        for metric, metric_label in [('nrmse', 'NRMSE'), ('smape', 'SMAPE'), ('mae', 'MAE')]:
            best = min(filtered, key=lambda x: x['metrics'][metric])
            val = best['metrics'][metric]
            if metric == 'mae':
                val = val * 1000
                metric_label = 'MAE (x1000)'

            fc_dim = best['config'].get('fc_hidden_dim', 'N/A')
            n_conv = best['config'].get('num_conv_layers', 'N/A')
            n_fc = best['config'].get('num_fc_layers', 'N/A')
            graph_mode = best['config'].get('graph_mode', 'N/A')

            lines.append(f"  Best {metric_label}: {val:.4f}")
            lines.append(f"    -> fc_dim={fc_dim}, n_conv={n_conv}, n_fc={n_fc}, mode={graph_mode}")

        lines.append("")

    # ========================================
    # Section 3: Best configuration for each fc_hidden_dim
    # ========================================
    lines.append("=" * 100)
    lines.append("BEST CONFIGURATION FOR EACH fc_hidden_dim")
    lines.append("=" * 100)
    lines.append("")
    ...
    for fc_dim in fc_dims:
        ...
        lines.append(f"--- fc_hidden_dim = {fc_dim} ({len(filtered)} configurations) ---")
        lines.append("")
        ...
        for metric, metric_label in [('nrmse', 'NRMSE'), ('smape', 'SMAPE'), ('mae', 'MAE')]:
            ...
            conv_dim = best['config'].get('conv_hidden_dim', 'N/A')
            n_conv = best['config'].get('num_conv_layers', 'N/A')
            n_fc = best['config'].get('num_fc_layers', 'N/A')
            graph_mode = best['config'].get('graph_mode', 'N/A')

            lines.append(f"  Best {metric_label}: {val:.4f}")
            lines.append(f"    -> conv_dim={conv_dim}, n_conv={n_conv}, n_fc={n_fc}, mode={graph_mode}")

        lines.append("")

    # ========================================
    # Section 4: Trend summary (average by parameter)
    # ========================================
    lines.append("=" * 100)
    lines.append("AVERAGE METRICS BY PARAMETER VALUE")
    lines.append("=" * 100)
    lines.append("")

    # Average by conv_hidden_dim
    lines.append("--- Average by conv_hidden_dim ---")
    lines.append(f"{'conv_dim':<12} {'Avg NRMSE':<15} {'Avg SMAPE':<15} {'Avg MAE(x1000)':<15} {'Count'}")
    lines.append("-" * 70)
    ...
    for conv_dim in conv_dims:
        ...
        if not filtered:
            continue
        avg_nrmse = np.mean([r['metrics']['nrmse'] for r in filtered])
        avg_smape = np.mean([r['metrics']['smape'] for r in filtered])
        avg_mae = np.mean([r['metrics']['mae'] for r in filtered]) * 1000
        lines.append(f"{conv_dim:<12} {avg_nrmse:<15.4f} {avg_smape:<15.4f} {avg_mae:<15.4f} {len(filtered)}")

    lines.append("")

    # Average by fc_hidden_dim
    lines.append("--- Average by fc_hidden_dim ---")
    lines.append(f"{'fc_dim':<12} {'Avg NRMSE':<15} {'Avg SMAPE':<15} {'Avg MAE(x1000)':<15} {'Count'}")
    lines.append("-" * 70)
    ...
    for fc_dim in fc_dims:
        ...
        avg_smape = np.mean([r['metrics']['smape'] for r in filtered])
        avg_mae = np.mean([r['metrics']['mae'] for r in filtered]) * 1000
        lines.append(f"{fc_dim:<12} {avg_nrmse:<15.4f} {avg_smape:<15.4f} {avg_mae:<15.4f} {len(filtered)}")

    lines.append("")

    # Average by num_conv_layers
    lines.append("--- Average by num_conv_layers ---")
    lines.append(f"{'n_conv':<12} {'Avg NRMSE':<15} {'Avg SMAPE':<15} {'Avg MAE(x1000)':<15} {'Count'}")
    lines.append("-" * 70)

    for n_conv in num_conv_layers_list:
        filtered = [r for r in results if r['config'].get('num_conv_layers') == n_conv]
        if not filtered:
            continue
        avg_nrmse = np.mean([r['metrics']['nrmse'] for r in filtered])
        avg_smape = np.mean([r['metrics']['smape'] for r in filtered])
        avg_mae = np.mean([r['metrics']['mae'] for r in filtered]) * 1000
        lines.append(f"{n_conv:<12} {avg_nrmse:<15.4f} {avg_smape:<15.4f} {avg_mae:<15.4f} {len(filtered)}")

    lines.append("")

    # Average by num_fc_layers
    lines.append("--- Average by num_fc_layers ---")
    lines.append(f"{'n_fc':<12} {'Avg NRMSE':<15} {'Avg SMAPE':<15} {'Avg MAE(x1000)':<15} {'Count'}")
    lines.append("-" * 70)

    for n_fc in num_fc_layers_list:
        filtered = [r for r in results if r['config'].get('num_fc_layers') == n_fc]
        if not filtered:
            continue
        avg_nrmse = np.mean([r['metrics']['nrmse'] for r in filtered])
        avg_smape = np.mean([r['metrics']['smape'] for r in filtered])
        avg_mae = np.mean([r['metrics']['mae'] for r in filtered]) * 1000
        lines.append(f"{n_fc:<12} {avg_nrmse:<15.4f} {avg_smape:<15.4f} {avg_mae:<15.4f} {len(filtered)}")

    lines.append("")

    # ========================================
    # Section 5: FG vs SA Comparison (same model size)
    # ========================================
    lines.append("=" * 100)
    ...
    # Group results by architecture (conv_dim, fc_dim, n_conv, n_fc)
    arch_groups = {}
    for r in results:
    ...
    # Header
    lines.append(f"{'conv_dim':<10} {'fc_dim':<10} {'n_conv':<8} {'n_fc':<8} | {'FG NRMSE':<12} {'SA NRMSE':<12} {'Winner':<8} | {'FG SMAPE':<12} {'SA SMAPE':<12} {'Winner':<8} | {'FG MAE':<12} {'SA MAE':<12} {'Winner':<8}")
    lines.append("-" * 160)
    ...
    for arch_key in sorted_archs:
        ...
        # Skip if only one mode available
        ...
        fg_smape = fg_result['metrics']['smape']
        sa_smape = sa_result['metrics']['smape']
        fg_mae = fg_result['metrics']['mae'] * 1000
        sa_mae = sa_result['metrics']['mae'] * 1000

        # Determine winners (lower is better)
        nrmse_winner = 'FG' if fg_nrmse < sa_nrmse else 'SA'
        smape_winner = 'FG' if fg_smape < sa_smape else 'SA'
        mae_winner = 'FG' if fg_mae < sa_mae else 'SA'
        ...
        line = f"{conv_dim:<10} {fc_dim:<10} {n_conv:<8} {n_fc:<8} | {fg_nrmse:<12.4f} {sa_nrmse:<12.4f} {nrmse_winner:<8} | {fg_smape:<12.4f} {sa_smape:<12.4f} {smape_winner:<8} | {fg_mae:<12.4f} {sa_mae:<12.4f} {mae_winner:<8}"
        lines.append(line)

    lines.append("-" * 160)
    lines.append("")
    lines.append(f"Total architectures with both FG and SA: {both_available}")
    lines.append("")
    lines.append("Win Summary:")
    lines.append(f"  NRMSE: FG wins {fg_wins_nrmse}, SA wins {sa_wins_nrmse}")
    lines.append(f"  SMAPE: FG wins {fg_wins_smape}, SA wins {sa_wins_smape}")
    lines.append(f"  MAE:   FG wins {fg_wins_mae}, SA wins {sa_wins_mae}")
    lines.append("")

    # Calculate average improvement
    if both_available > 0:
        lines.append("Average Metrics:")
        fg_avg_nrmse = np.mean([arch_groups[k]['FG']['metrics']['nrmse'] for k in sorted_archs if arch_groups[k]['FG'] and arch_groups[k]['SA']])
        sa_avg_nrmse = np.mean([arch_groups[k]['SA']['metrics']['nrmse'] for k in sorted_archs if arch_groups[k]['FG'] and arch_groups[k]['SA']])
        fg_avg_smape = np.mean([arch_groups[k]['FG']['metrics']['smape'] for k in sorted_archs if arch_groups[k]['FG'] and arch_groups[k]['SA']])
        sa_avg_smape = np.mean([arch_groups[k]['SA']['metrics']['smape'] for k in sorted_archs if arch_groups[k]['FG'] and arch_groups[k]['SA']])
        fg_avg_mae = np.mean([arch_groups[k]['FG']['metrics']['mae'] for k in sorted_archs if arch_groups[k]['FG'] and arch_groups[k]['SA']]) * 1000
        sa_avg_mae = np.mean([arch_groups[k]['SA']['metrics']['mae'] for k in sorted_archs if arch_groups[k]['FG'] and arch_groups[k]['SA']]) * 1000

        lines.append(f"  FG Average - NRMSE: {fg_avg_nrmse:.4f}, SMAPE: {fg_avg_smape:.4f}, MAE(x1000): {fg_avg_mae:.4f}")
        lines.append(f"  SA Average - NRMSE: {sa_avg_nrmse:.4f}, SMAPE: {sa_avg_smape:.4f}, MAE(x1000): {sa_avg_mae:.4f}")

        nrmse_diff = ((sa_avg_nrmse - fg_avg_nrmse) / sa_avg_nrmse) * 100
        smape_diff = ((sa_avg_smape - fg_avg_smape) / sa_avg_smape) * 100
        mae_diff = ((sa_avg_mae - fg_avg_mae) / sa_avg_mae) * 100

        lines.append(f"  FG vs SA Improvement: NRMSE {nrmse_diff:+.2f}%, SMAPE {smape_diff:+.2f}%, MAE {mae_diff:+.2f}%")
        lines.append(f"  (Positive = FG is better, Negative = SA is better)")
...
def plot_temperature_comparison(results, output_dir):
    """Plot metrics across different temperatures (TSMC specific)."""
    if not results:
        return
    ...
    temp_groups = {}
    for r in results:
        temp = r['config'].get('temperature', None)
        if temp is not None:
    ...
    if len(temp_groups) < 2:
        print("Not enough temperature groups for temperature comparison plot")
        return
    ...
    # NRMSE by temperature
    ax = axes[0]
    nrmse_by_temp = []
    for temp in temps:
        values = [r['metrics']['nrmse'] for r in temp_groups[temp]]
        nrmse_by_temp.append(values)

    bp = ax.boxplot(nrmse_by_temp, labels=[f'{t}C' for t in temps], patch_artist=True)
    for patch in bp['boxes']:
        patch.set_facecolor('lightblue')
    ax.set_xlabel('Temperature')
    ax.set_ylabel('NRMSE (%)')
    ax.set_title('NRMSE Distribution by Temperature')
    ax.grid(True, alpha=0.3)
    ...
    mape_by_temp = []
    for temp in temps:
        values = [r['metrics']['mape'] for r in temp_groups[temp]]
        mape_by_temp.append(values)

    bp = ax.boxplot(mape_by_temp, labels=[f'{t}C' for t in temps], patch_artist=True)
    for patch in bp['boxes']:
        patch.set_facecolor('lightgreen')
    ax.set_xlabel('Temperature')
    ax.set_ylabel('MAPE (%)')
    ax.set_title('MAPE Distribution by Temperature')
    ax.grid(True, alpha=0.3)

    plt.suptitle('Metrics by Temperature (TSMC)', fontsize=14)
    plt.tight_layout()

    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, 'temperature_comparison.png')
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    print(f"Saved: {output_path}")
    plt.close()


def plot_corner_comparison(results, output_dir):
    """Plot metrics across different corners (TSMC specific)."""
    if not results:
        return
    ...
    corner_groups = {}
    for r in results:
        corner = r['config'].get('corner', None)
        if corner is not None:
            if corner not in corner_groups:
                corner_groups[corner] = []
            corner_groups[corner].append(r)

    if len(corner_groups) < 2:
        print("Not enough corner groups for corner comparison plot")
        return

    _fig, axes = plt.subplots(1, 2, figsize=(14, 5))

    corners = sorted(corner_groups.keys())

    # NRMSE by corner
    ax = axes[0]
    nrmse_by_corner = []
    for corner in corners:
        values = [r['metrics']['nrmse'] for r in corner_groups[corner]]
        nrmse_by_corner.append(values)

    bp = ax.boxplot(nrmse_by_corner, labels=corners, patch_artist=True)
    colors = ['lightcoral', 'lightyellow', 'lightblue']
    for patch, color in zip(bp['boxes'], colors[:len(corners)]):
        patch.set_facecolor(color)
    ax.set_xlabel('Corner')
    ax.set_ylabel('NRMSE (%)')
    ax.set_title('NRMSE Distribution by Corner')
    ax.grid(True, alpha=0.3)
    ...
    mape_by_corner = []
    for corner in corners:
        values = [r['metrics']['mape'] for r in corner_groups[corner]]
        mape_by_corner.append(values)

    bp = ax.boxplot(mape_by_corner, labels=corners, patch_artist=True)
    for patch, color in zip(bp['boxes'], colors[:len(corners)]):
        patch.set_facecolor(color)
    ax.set_xlabel('Corner')
    ax.set_ylabel('MAPE (%)')
    ax.set_title('MAPE Distribution by Corner')
    ax.grid(True, alpha=0.3)

    plt.suptitle('Metrics by Corner (TSMC)', fontsize=14)
    plt.tight_layout()

    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, 'corner_comparison.png')
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    print(f"Saved: {output_path}")
    plt.close()
...
def print_results_table(results, output_file=None):
    ...
    lines.append("\n" + "=" * 120)
    lines.append("TSMC GNN VALIDATION RESULTS COMPARISON")
    lines.append("=" * 120)
    lines.append("Architecture format: [Model]_[Graph]_[Corner][Temp]_[ConvDim]x[ConvLayers]_[FCDim]x[FCLayers]")
    lines.append("  Model: B=Baseline, M=MAML  |  Graph: FG=full_graph, SA=stage_aware")
    lines.append("-" * 120)
    lines.append(f"{'Architecture':<40} {'Corner':<8} {'Temp':<6} {'NRMSE%':<10} {'SMAPE%':<10} {'MAE(x1000)':<12} {'R2':<10} {'Samples':<10}")
    lines.append("-" * 120)

    for r in results:
        arch = r['config'].get('arch_label', 'unknown')[:35]
        corner = r['config'].get('corner', '?')
        temp = r['config'].get('temperature', '?')
        nrmse = r['metrics']['nrmse']
        smape = r['metrics']['smape']
        mae = r['metrics']['mae'] * 1000  # x1000
        r2 = r['metrics']['r2']
        samples = r['metrics']['num_samples']

        lines.append(f"{arch:<40} {corner:<8} {temp:<6} {nrmse:<10.4f} {smape:<10.4f} {mae:<12.4f} {r2:<10.4f} {samples:<10}")
    ...
    lines.append(f"  Corner: {best['config'].get('corner', '?')}")
    lines.append(f"  Temperature: {best['config'].get('temperature', '?')}C")
    lines.append(f"  NRMSE: {best['metrics']['nrmse']:.4f}%")
    lines.append(f"  SMAPE: {best['metrics']['smape']:.4f}%")
    lines.append(f"  MAE (x1000): {best['metrics']['mae']*1000:.4f}")
    lines.append(f"  R2: {best['metrics']['r2']:.4f}")

    # Summary by corner
    lines.append("\n" + "-" * 60)
    lines.append("Summary by Corner:")
    corner_stats = {}
    for r in results:
        corner = r['config'].get('corner', 'unknown')
        if corner not in corner_stats:
            corner_stats[corner] = {'nrmse': [], 'smape': [], 'mae': []}
        corner_stats[corner]['nrmse'].append(r['metrics']['nrmse'])
        corner_stats[corner]['smape'].append(r['metrics']['smape'])
        corner_stats[corner]['mae'].append(r['metrics']['mae'])

    for corner in sorted(corner_stats.keys()):
        stats = corner_stats[corner]
        lines.append(f"  {corner}: NRMSE={np.mean(stats['nrmse']):.4f}%, SMAPE={np.mean(stats['smape']):.4f}%, MAE(x1000)={np.mean(stats['mae'])*1000:.4f} (n={len(stats['nrmse'])})")
...
def main():
    parser = argparse.ArgumentParser(
        description='Compare and visualize GNN validation sweep results for TSMC dataset',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    python compare_gcn_validation_results_tsmc.py --results_dir 10000samples --mode interpolation
    python compare_gcn_validation_results_tsmc.py --results_dir 10000samples --mode extrapolation
    python compare_gcn_validation_results_tsmc.py --results_dir 10000samples --mode interpolation --filter "FF_0"
    python compare_gcn_validation_results_tsmc.py --results_dir 10000samples --mode interpolation --corner FF
    python compare_gcn_validation_results_tsmc.py --results_dir 10000samples --mode interpolation --temperature 25

    # With --corner and --temperature: generates both normal + cropped plots automatically
    python compare_gcn_validation_results_tsmc.py --results_dir 10000samples --mode interpolation --corner FF --temperature 0
"""
    )
    ...
    parser.add_argument('--output_dir', type=str,
                       default='/home/tkdgn2907/Deepsets_test/MAML/Projects/result_management/result_summary/gcn_validation_analysis_tsmc',
    ...
    parser.add_argument('--data_type', type=str, default='cell',
                       ...
                       help='Data type for MLP MAML baseline comparison: cell (default) or transition')
    ...
    if args.temperature is not None:
        results = [r for r in results if r['config'].get('temperature') == args.temperature]
        print(f"Filtered to {len(results)} results with temperature={args.temperature}")
    ...
    subdir_name = f"tsmc_{args.mode}"
    if args.corner:
        subdir_name += f"_{args.corner}"
    if args.temperature is not None:
        subdir_name += f"_{args.temperature}"
    if args.filter:
    ...
    if not args.no_plots:
        ...
        # Load MLP MAML baseline if corner and temperature are specified
        mlp_maml_baseline = None
        if args.corner and args.temperature is not None:
            print(f"\nLoading MLP MAML baseline for {args.corner}_{args.temperature} ({args.data_type})...")
            mlp_maml_baseline = load_mlp_maml_baseline(args.corner, args.temperature, data_type=args.data_type)
        ...
        def generate_all_plots(results_to_use, out_dir, baseline, crop=False):
            ...
            plot_temperature_comparison(results_to_use, out_dir)
            plot_corner_comparison(results_to_use, out_dir)
